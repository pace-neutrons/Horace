...
for i = 1:numel(din)
    ...
    %------ some eror catches, libisis can not have empty titles and axes
    % labels ----% 
    
    if isempty(din(i).title)
        din(i).title = ' ';
    end
    
    if isempty(din(i).label(din(i).pax(1)))
        din(i).label(din(i).pax(1)) = ' ';
    end
    
    if isempty(din(i).label(din(i).pax(2)))
        din(i).label(din(i).pax(2)) = ' ';
    end
    
    if isempty(din(i).label(din(i).pax(3)))
        din(i).label(din(i).pax(3)) = ' ';
    end
    
    % ----end---- %
    
    dout(i) = IXTdataset_3d(IXTbase('unknown', false, true), din.title, din(i).s, sqrt(din(i).e), ... 
