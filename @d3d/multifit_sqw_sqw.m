...
% Simultaneously fits a model for S(Q,w) to an array of d3d objects, with 
...
% 
% Differs from fit_sqw_sqw, which independently fits each dataset in 
% succession. 
% 
% Simultaneously fit datasets to a single function ('global foreground'): 
% ----------------------------------------------------------------------- 
%   >> [wout, fitdata] = multifit_sqw_sqw (w, func, pin) 
%   >> [wout, fitdata] = multifit_sqw_sqw (w, func, pin, pfree) 
%   >> [wout, fitdata] = multifit_sqw_sqw (w, func, pin, pfree, pbind) 
% 
...
%   >> [wout, fitdata] = multifit_sqw_sqw (..., 'local_foreground') 
% 
...
%   >> [wout, fitdata] = multifit_sqw_sqw (..., 'global_background') 
% 
% 
...
% 
%   EXAMPLES: 
%   >> [wout, fitdata] = multifit_sqw_sqw(...,'keep',[0.4,1.8],'list',2) 
% 
%   >> [wout, fitdata] = multifit_sqw_sqw(...,'select') 
% 
...
% ========= 
% 
% The examples for d1d objects illustrate the use of multifit_sqw_sqw, 
% although the dimensionality of the examples is different. Type 
% >> help d1d/multifit_sqw_sqw  and look at the examples. 
 
% <#doc_def:> 
%   first_line = {'% Simultaneously fits a model for S(Q,w) to an array of d3d objects, with',... 
%                 '% optional background functions that are also models for S(Q,w).'} 
%   main = false; 
%   method = true; 
%   synonymous = false; 
