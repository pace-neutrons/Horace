...
% Fit a model for S(Q,w) to a d3d object, with an optional background function. 
% 
...
% 
% Differs from multifit_sqw, which fits all datasets in the array 
% simultaneously but with independent backgrounds. 
% 
% Fit several datasets in succession to a given function: 
% ------------------------------------------------------- 
%   >> [wout, fitdata] = fit_sqw (w, func, pin) 
%   >> [wout, fitdata] = fit_sqw (w, func, pin, pfree) 
%   >> [wout, fitdata] = fit_sqw (w, func, pin, pfree, pbind) 
% 
...
% 
%   EXAMPLES: 
%   >> [wout, fitdata] = fit_sqw(...,'keep',[0.4,1.8],'list',2) 
% 
%   >> [wout, fitdata] = fit_sqw(...,'select') 
% 
...
% ========= 
% 
% The examples for d1d objects illustrate the use of fit_sqw, although the 
% dimensionality of the examples is different. Type >> help d1d/fit_sqw 
% and look at the examples. 
 
% <#doc_def:> 
%   first_line = {'% Fit a model for S(Q,w) to a d3d object, with an optional background function.'} 
%   main = false; 
%   method = true; 
%   synonymous = false; 
% 
