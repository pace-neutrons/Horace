classdef hlp_serial_types
    ...
    properties(Constant)
        ...
        type_details = struct('name',...
            ...
            'size',... % Sizes of respective simple data types. Complex data types are calculated so their basic type is 0;
        ...
        % function handle type constist of common tag and subtag. One more
        % function handle type available (0)
        func_handle_types ={'simple', 'classsimple','anonymous','scopedfunction','nested'}
        func_handle_tags  ={ 64            64          128           192            192  }
    ...
    methods(Static)
        ...
        %
        function [type_str, nDims,siz_or_fhid,pos] = unpack_data_tag(head_bytes,pos)
            ...
            % siz_or_fhid-- the array of size and shape of the encoded
            %               object. 
            %            -- As function handle is always size 1, this
            %               field contains the type of the function handle
            %               code if type of header is function_handle
            % pos
            tag = uint8(head_bytes(pos));
            if tag>63 % function handle specific types.
                fh_tag = bitshift(tag,-6);
                fh_tag = bitshift(fh_tag,6);
                tag    = tag-fh_tag;
                type_str = hlp_serial_types.type_details(tag + 1);
                nDims=1;
                siz_or_fhid =double(fh_tag);
                pos = pos+1;
                return;
            end
            ...
            elseif nDims == 1
                ...
                nBytes = nDims*4;
                siz_or_fhid   = double(typecast(head_bytes(pos:pos+nBytes-1), 'uint32')');
                pos    = pos+nBytes;
            end
        ...
        function comb_tag = pack_data_tag(data_size,type_struc,varargin)
            ...
            %               handle to process. Ignored in all other cases
            % Returns:
            ...
            elseif nDims == 2 && data_size(1) == 1 % some saving in writing rows
                %  assuming 2D arrays are rows. If this incorrect,
                %  use general case.
                comb_tag = [tag;hlp_serial_types.dims_tag(1);...
        ...
        function tag_size = calc_tag_size(data_size,type_struc,varargin)
            ...
            elseif nDims == 2 && data_size(1) == 1 % some saving in writing rows
                % if incoming 2D arrays are rows. Go to general case
                % otherwise
                tag_size = hlp_serial_types.tag_size+4;
            else
