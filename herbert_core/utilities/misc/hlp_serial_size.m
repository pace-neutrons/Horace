...
sz = hlp_serial_types.tag_size+hlp_serial_types.get_size(class(v));
end
...
if size(v,1) == 1
    ...
    sz = hlp_serial_types.tag_size+hlp_serial_types.dim_size+length(v);
    %m = [uint8(0); typecast(uint32(length(v)),'uint8').'; uint8(v(:))];
elseif sum(size(v)) == 0
    % '': special encoding
    sz = hlp_serial_types.tag_size;
else
    % general char array: Tag & Number of dimensions, Dimensions, Data
    sz = hlp_serial_types.tag_size + hlp_serial_types.ndims_size +...
        hlp_serial_types.dim_size*numel(size(v))+numel(v);
...
sz = hlp_serial_types.tag_size + hlp_serial_types.ndims_size +...
    hlp_serial_types.dim_size*numel(size(v))+numel(v)*bs;
end
...
if issparse(v)
    ...
    % Sparses use uint64 sized indices
    sz = hlp_serial_types.tag_size+hlp_serial_types.get_size('uint64')+hlp_serial_types.get_size('uint64');
    % Index vectors
    [i,j,s] = find(v);
    % Real/Complex
    if isreal(v)
        sz = sz+serial_size_numeric_simple(i)+serial_size_numeric_simple(j)+hlp_serial_types.tag_size+...
            serial_size_numeric_simple(s);
        %m = [m; serialize_numeric_simple(i); serialize_numeric_simple(j); 1; serialize_numeric_simple(s)];
    else
        sz = sz+serial_size_numeric_simple(i)+serial_size_numeric_simple(j)+hlp_serial_types.tag_size+...
            serial_size_numeric_simple(real(s))+serial_size_numeric_simple(imag(s));
...
elseif ~isreal(v)
    ...
    sz = hlp_serial_types.tag_size+serial_size_numeric_simple(real(v))+serial_size_numeric_simple(imag(v));
elseif isscalar(v)
    % Scalar
...
fnChars = [fieldNames{:}];
dims = [ndims(v) size(v)];
sz = hlp_serial_types.tag_size+hlp_serial_types.dim_size*numel(fnLengths)+numel(fnChars)+hlp_serial_types.dim_size*numel(dims);
...
if numel(v) > length(fieldNames)
    ...
    sz = sz+hlp_serial_types.tag_size+numel(tmp);
    %m = [m; 0; vertcat(tmp{:})];
else
    % more field names than records; use struct2cell
    sz = sz+hlp_serial_types.tag_size+serial_size_cell(struct2cell(v));
...
sz = hlp_serial_types.tag_size + hlp_serial_types.ndims_size +...
    hlp_serial_types.dim_size*numel(size(v))+sum(reshape(contents,1,numel(contents)));
...
sz =hlp_serial_types.tag_size + hlp_serial_types.ndims_size +...
    hlp_serial_types.dim_size*numel(size(v))+sum(contents);
%m = [uint8(33); ndims(v); typecast(uint32(size(v)),'uint8').'; vertcat(contents{:})];
end
...
if sizeprod == 1
    ...
    if (all(cellfun('isclass',v(:),'double')) || all(cellfun('isclass',v(:),'single'))) && all(~cellfun(@issparse,v(:)))
        ...
        if reality
            ...
            sz = hlp_serial_types.tag_size + serial_size_numeric_simple(reshape([v{:}],size(v)));
        elseif ~reality
            % all complex
            %m = [uint8(34); serialize_numeric(reshape([v{:}],size(v)))];
            sz = hlp_serial_types.tag_size + serial_size_numeric(reshape([v{:}],size(v)));
        else
            % mixed reality
            %m = [uint8(35); serialize_numeric(reshape([v{:}],size(v))); serialize_logical(reality(:))];
            sz = hlp_serial_types.tag_size + serial_size_numeric(reshape([v{:}],size(v)))+serial_size_logical(reality(:));
        end
    else
        % non-float types
        ...
        elseif cellfun('isclass',v,'logical')
            ...
            sz = hlp_serial_types.tag_size + serial_size_logical(reshape([v{:}],size(v)));
...
elseif isempty(v)
    ...
    sz = hlp_serial_types.tag_size + hlp_serial_types.ndims_size +...
        hlp_serial_types.dim_size*numel(size(v));
else
    ...
    if cellfun('isclass',v,'char') & size1 <= 1 %#ok<AND2>
        ...
        sz = hlp_serial_types.tag_size+serial_size_string([v{:}])+serial_size_numeric_simple(uint32(size2))+serial_size_logical(size1(:)==0);
    elseif (size1+size2 == 0) & (dims == 2) %#ok<AND2>
        ...
        if all(cellfun('isclass',v(:),'double')) || all(cellfun('isclass',v(:),'cell')) || all(cellfun('isclass',v(:),'struct'))
            ...
            sz = hlp_serial_types.tag_size + hlp_serial_types.tag_size + hlp_serial_types.ndims_size +...
                hlp_serial_types.dim_size*numel(size(v));
        elseif length(unique(cellfun(@class,v(:),'UniformOutput',false))) == 1
            ...
            sz = hlp_serial_types.tag_size + hlp_serial_size(class(v{1})) + hlp_serial_types.ndims_size +...
                hlp_serial_types.dim_size*numel(size(v));
        else
...
if ismethod(v, 'serialize')
    ...
    sz = hlp_serial_types.tag_size + serial_size_string(class(v)) + sz;
...
switch rep.type
    case 'simple'
        ...
        sz = hlp_serial_types.tag_size + serial_size_string(rep.function);
    ...
    case 'anonymous'
        ...
        if isfield(tracking,'serialize_anonymous_fully') && tracking.serialize_anonymous_fully
            ...
            % Tag and Code
            sz = hlp_serial_types.tag_size + serial_size_string(char(v));
            ...
            if ~isempty(rep.workspace)
                sz = hlp_serial_types.tag_size + serial_size_string(char(v)) + serial_size_struct(rep.workspace{1});
                %m = [uint8(152); serialize_string(char(v)); serialize_struct(rep.workspace{1})];
            else
                sz = hlp_serial_types.tag_size + serialize_string(char(v)) + serial_size_struct(struct());
    ...
    case {'scopedfunction','nested'}
        ...
        sz = hlp_serial_types.tag_size + serial_size_cell(rep.parentage);
    otherwise
