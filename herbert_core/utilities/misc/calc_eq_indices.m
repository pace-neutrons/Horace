...
% error   -- 2 element array containing absolute and relative error. If the
%            difference between two elemens of data is smaller then
...
%             all data indices. I.e. if there are 10 data
%             elements, eq_idx == {1:10}. If eq_idx contain two
%             different values, first five A1 and second five - A2;
%             eq_idx == {1:5,6:10}. All different data will return
%             cellarray eq_idx = {1,2,3,...9,10};
...
for i = 1:n_elements
    ...
    for j=i+1:n_elements
        same = calc_err(data{i},data{j},error);
        if same
            ic = ic+1;
            eq_bunch(ic) = j;
            iseq(j) = true;
...
function same = calc_err(efix1,efix2,Diff)
same = true;
