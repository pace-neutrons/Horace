classdef ClusterSlurm < ClusterWrapper
    ...
    properties(Access=protected)
        ...
        % Time to keep cluster alive without activity
        keep_cluster_alive_ = 300;
    end

    properties(Access=private)
        % timeout timer to ensure job closes after period of inactivity
        timer_ = [];
    end
    ...
    methods
        function obj = ClusterSlurm(n_workers,mess_exchange_framework,log_level)
            % Constructor, which initiates Slurm MPI wrapper
            %
        ...
        function obj = start_job(obj,je_init_message,task_init_mess,log_message_prefix)
            ...
            obj = start_job@ClusterWrapper(obj,je_init_message,task_init_mess,log_message_prefix);
        ...
        function [completed, obj] = check_progress(obj,varargin)
            ...
            % If we're done we're on a timer to killing the process
            if completed
                t = timer;
