...
obj = IX_divergence_profile();  % default instance of the object
...
%               there is now also the private property class_version_, and
%               public property 'angle' is now public 'angles' and private
%               'angles_'
...
if isfield(S,'class_version_')
    ...
    % which can be set in any order, except 'angle' is now 'angles'.
    % The public (i.e. dependent in this case) properties cannot be set 
    % so must use the class constructor
    
    if numel(nams)==3 && all(isfield(S,{'name','angle','profile'}))
        obj = IX_divergence_profile(S.name, S.angle, S.profile);
    else
