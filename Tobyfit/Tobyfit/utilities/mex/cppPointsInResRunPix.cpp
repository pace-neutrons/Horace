...
template <typename T, typename R> T points_in_resolution_pixel(
	...
	const R *Y, const T *Yrun, const T *Yhead, const T* Ylist,
	const R *X, const R *M, const R fracV, const T run,
	T *YiX, R *VxR) {
	...
	*/
	...
	for (T i = 0; i < nNeighbourCells; i++) {
		...
		for (T j = Yhead[neighbourCells[i]]; j > 0; j = Ylist[j-1]) {
			if (Yrun[j-1]==run){ // delta(Yrun,Xrun)
...
template <typename T, typename R> T points_in_resolution(
	...
	const T n, const R *Y, const T* Yrun, const T *Yhead, const T *Ylist,
	const T m, const R *X, const R *M, const  R *V, const T *r, const  T *c, const R frac,
	...
	shared(cells,spans,Y,Yrun,Yhead,Ylist,X,M,V,r,c,nYiX,allYiX,allVxR,subtotal)
	{
	...
	for (int i=0; i<m; i++){
		...
		pir=points_in_resolution_pixel(d,nNeighbours,neighbourCells,Y,Yrun,Yhead,Ylist,X+d*i,M+dd*i,frac*V[i],r[i],tYiX,tVxR);
...
int test_points_in_resolution_pixel(int depth){
    ...
    double Y[6]={1.,0.,0.,6.,0.,1.};
		int Yrun[3]={1,1,1}, Yhead[1]={3}, Ylist[3]={0,1,2};
    ...
    double VxR[3];
		...
		nYiX = points_in_resolution_pixel(d,nnc,nc,Y,Yrun,Yhead,Ylist,X,M,fracV,Xrun,YiX,VxR);
		if (nYiX!=2) return -1;
		...
		fracV = frac*V;
		nYiX = points_in_resolution_pixel(d,nnc,nc,Y1,Yrun,Yhead,Ylist,X1,M1,fracV,Xrun,YiX,VxR);
		if (2!=nYiX) return -1;
		if (2!=YiX[0]) return -2;
		if (1!=YiX[1]) return -2;
...
int test_points_in_resolution(int depth){
	...
	int Yrun[5]={1,2,1,1,2}, Xrun[2]={1,2};
	...
	int total = points_in_resolution(d,cells,spans,n,Y,Yrun,Yhead,Ylist,m,X,M,V,Xrun,Xcell,frac,nYiX,allYiX,allVxR);
  std::cout << spacepad(depth) << "found " << total << " points in resolution. Expected 5" << std::endl;
	if (5!=total) return -1;
	for (int i=0; i<2; i++){
	...
	if (3!=nYiX[0]) return -2;
	if (4!=allYiX[0][0]) return -3;
	if (3!=allYiX[0][1]) return -3;
	if (1!=allYiX[0][2]) return -3;
...
3	Y		the point(s) to check: (dim,n)
4 Yrun	the run number associated with each point. (1,n)
5	Yhead	the head of a linked list putting points into cells: (1,prod(nCell))
6	Ylist	the list of a linked list putting points into cells: (1,n)

7	X		the pixel(s) to check: (dim,m)
8	M		the pixel resolution gaussian widths: (dim,dim,m)
9	V		the pixel resolution volumes: (1,m)
10 Xrun 	the run number for each pixel: (m,1)
11 Xcell 	the cell index for each pixel: (m,1)

12	frac	the fractional probability for deciding if a point is within resolution
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (nrhs == 0){
	...
	if (nrhs != 13) mexErrMsgIdAndTxt("HORACE:cPointsInResolution:nrhs", "Thirteen input aruments expected.");
	...
	int intInputs[7] = { 1, 2, 4, 5, 6, 10, 11 };
	for (int i=0; i < 6; i++) {
		...
		if (mxGetN(prhs[intInputs[i]]) != 1) {
			char *buf = new char[100]();
			sprintf(buf,"Input %d expected to be a column vector",intInputs[i]+1);
			mexErrMsgIdAndTxt("HORACE:cPointsInResolution:rowVector", buf);
		}
	}

	int dblInputs[5] = { 3, 7, 8, 9, 12 };
	for (int i=0; i < 5; i++) {
	...
	if (mxGetM(prhs[9]) != 1) mexErrMsgIdAndTxt("HORACE:cPointsInResolution:rowVector", "V is expected to be a row vector");
	if (!mxIsScalar(prhs[12])) mexErrMsgIdAndTxt("HORACE:cPointsInResolution:notScalar", "The frac input must be scalar");
	...
	if (mxGetM(prhs[4]) != n) mexErrMsgIdAndTxt("HORACE:cPointsInResolution:dimensions", "The sizes of Y and Yrun are inconsistent.");
	if (mxGetM(prhs[5]) != t) mexErrMsgIdAndTxt("HORACE:cPointsInResolution:dimensions", "The size of Yhead is inconsistent with total.");
	if (mxGetM(prhs[6]) != n) mexErrMsgIdAndTxt("HORACE:cPointsInResolution:dimensions", "The sizes of Y and Ylist are inconsistent.");
	if (mxGetM(prhs[7]) != d) mexErrMsgIdAndTxt("HORACE:cPointsInResolution:dimensions", "The sizes of cell and X are inconsistent.");
	if (mxGetNumberOfElements(prhs[8]) != d * d * m) mexErrMsgIdAndTxt("HORACE:cPointsInResolution:dimensions", "The size of M is not consistent with the size of X");
	if (mxGetN(prhs[9]) != m) mexErrMsgIdAndTxt("HORACE:cPointsInResolution:dimensions", "The size of V is not consistent with the size of X");
	if (mxGetM(prhs[10]) != m) mexErrMsgIdAndTxt("HORACE:cPointsInResolution:dimensions", "The sizes of X and Xrun are inconsistent.");
	if (mxGetM(prhs[11]) != m) mexErrMsgIdAndTxt("HORACE:cPointsInResolution:dimensions", "The sizes of X and Xcell are inconsistent.");

	// get handles to array inputs
	intType *cells, *spans, *Yrun, *Yhead, *Ylist, *Xrun, *Xcell;
	cells = (intType *)mxGetData(prhs[1]);
	spans = (intType *)mxGetData(prhs[2]);
	Yrun  = (intType *)mxGetData(prhs[4]);
	Yhead = (intType *)mxGetData(prhs[5]);
	Ylist = (intType *)mxGetData(prhs[6]);
	Xrun  = (intType *)mxGetData(prhs[10]);
	Xcell = (intType *)mxGetData(prhs[11]);

	double *Y, *X, *M, *V;
	Y = (double *)mxGetData(prhs[3]);
	X = (double *)mxGetData(prhs[7]);
	M = (double *)mxGetData(prhs[8]);
	V = (double *)mxGetData(prhs[9]);

	// get the fractional probability input
	double frac;
	frac = mxGetScalar(prhs[12]);
	...
	total_in_resolution = points_in_resolution(d, cells, spans, n, Y, Yrun, Yhead, Ylist, m, X, M, V, Xrun, Xcell, frac, nYiX, allYiX, allVxR);
