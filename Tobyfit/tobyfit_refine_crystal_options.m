function [xtal_opts,ok,mess] = tobyfit_refine_crystal_options(varargin)
...
%
%   >> opts = tobyfit_refine_crystal_options (...)   % see below for useage
%   >> [wout,fit]=tobyfit(...,'refine_crystal',opts,...)
%
% Default options
%   >> opts = tobyfit_refine_crystal_options
%
% Set initial lattice parameters different to those in the sqw objects
%   >> opts = tobyfit_refine_crystal_options (alatt_init, angdeg_init)
%
% In addition, there are keyword arguments to control the refinement e.g.
%   >> opts = tobyfit_refine_crystal_options(..., 'fix_angdeg')
%   >> opts = tobyfit_refine_crystal_options(..., 'free_alatt', [1,0,1])
%
...
%                   If empty e.g. [] or '' or omitted then current values in sqw objects are used
...
%   vrot            Direction of y-axis for rotation matrix in r.l.u. (if not perpendicular to
%                  urot, then the y-axis will be constructed in the usual fashion as
%                  being in the plane of urot and vrot with vrot habving a positive
...
% Output:
% -------
%   opts            Structure with crystal refinement options
...
%   >> opts = tobyfit_refine_crystal_options('fix_lattice')
%
...
if nargin==1 && isstruct(varargin{1})   % input a single structure
    ...
    if numel(args)>=4, xtal_opts.urot=args{4};   end
    if numel(args)>=5
