function optimal_configuration(varargin)
%% Optimal configuration
% Determine optimal configuration for current machine.

p = inputParser();
p.addOptional('mem_pct', 0.6, @(x) validateAtrributes(x, {'numeric'}, {'scalar', 'positive'}));
p.addOptional('num_cores', feature('numcores'), ...
              @(x) validateAtrributes(x, {'numeric'}, {'scalar', 'integer', 'positive'}));
p.addOptional('system_memory', sys_memory(), ...
              @(x) validateAtrributes(x, {'numeric'}, {'scalar', 'positive'}));
p.addOptional('mpi', 'n/a', ...
              @(x) validatestring(x, {'mpiexec_mpi', ...
                                      'parpool', ...
                                      'herbert', ...
                                      'slurm'}))
p.addOptional('try_mex', true, @islognumscalar);
p.addOptional('dryrun', false, @islognumscalar);
p.parse(varargin{:});

if ~p.Results.dryrun
    hc = hor_config;
    pc = parallel_config;
    hpc = hpc_config;
else
    hc = struct();
    pc = struct();
    hpc = struct();
end

% Test mex availability
if p.Results.try_mex
    [~, n_errors, can_combine_with_mex] = check_horace_mex();
    use_mex = n_errors == 0;
    if n_errors == 0

        fprintf('- mex acceleration available, using mex.\n');
        hc.use_mex = true;

        if can_combine_with_mex
            fprintf('- Can combine with mex, enabling.\n');
            hc.combine_sqw_using = 'mex_code';
        end
    else
        fprintf('- mex acceleration not available. Consider building Horace with mex. \n');
        hor_config.use_mex = false;
        hc.combine_sqw_using = 'matlab';
    end
end

% Test CPU availability

if any(strcmp(p.UsingDefaults, 'num_cores'))
    fprintf(['- Discovered %d cores. Using all for calculations.\n', ...
             '  If you believe this number is not correct, ', ...
             'please call this function again with:\n\n', ...
             '  optimal_configuration(''num_cores'', <N>)\n'], p.Results.num_cores);
    pc.parallel_workers_number = p.Results.num_cores;
    pc.threads = 0;
    pc.parallel_threads = 0;
else
    pc.parallel_workers_number = p.Results.num_cores;
    pc.threads = p.Results.num_cores;
    pc.parallel_threads = 1;
end

% Set cluster configuration

% Test slurm availablility
[err_code, ~] = system('squeue');
if err_code == 0
    warning('HORACE:optimal_configuration:slurm_available', ...
            ['- Slurm may be available on your system\n', ...
             '  Horace can only configure for the local node. \n', ...
             '  You may need to manually configure your parallel_config ', ...
             'to take advantage of slurm'])
end

if hc.use_mex
    fprintf('- Using C++ MPI as mex available.\n')
    pc.parallel_cluster = 'mpiexec_mpi';
elseif contains(evalc('ver'), 'Parallel Computing Toolbox')
    fprintf('- Using parpool MPI as MATLAB Parallel Computing Toolbox Available.\n')
    pc.parallel_cluster = 'parpool';
else
    fprintf('- Using poor-man MPI as no other options available.\n', ...
            '  Consider compiling mex for better parallel performance.\n')
    pc.parallel_cluster = 'herbert';
end

% Test Memory availability
hc.mem_chunk_size = (p.Results.mem_pct * p.Results.system_memory) / ...
    (8 * PixelDataBase.DEFAULT_NUM_PIX_FIELDS);
hpc.mex_combine_buffer_size = (p.Results.mem_pct * p.Results.system_memory) / ...
    (pc.parallel_workers_number)
if any(strcmp(p.UsingDefaults, 'system_memory'))
    fprintf('- Detected %0.1fGb of memory. \n', p.Results.system_memory / (1024.^3))
    fprintf('  Using %0.1f%% (%d) for page size.\n', 100 * p.Results.mem_pct, hc.mem_chunk_size)
    if hc.use_mex
        fprintf('  Using %0.1f%% per worker (%d) for mex combine buffer.\n', 100 * p.Results.mem_pct / pc.parallel_workers_number, hpc.mex_combine_buffer_size)
    end

    fprintf(['  If you believe this number is nor correct, ', ...
             'please call this function again with:\n\n', ...
             '  optimal_configuration(''mem_pct'', <X>, ''system_memory'', <Y>)'])
end
hc.fb_scale_factor = 2;

if p.Results.dryrun
    hc
    pc
    hpc
end

end
