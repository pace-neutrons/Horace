...
*  This means assigning them values which are completely invlid, namely 
...
size_t bin_pixels(double* const npix, double* const s, double* const e, BinningArg* const bin_par_ptr)
{
    ...
    for (long i = 0; i < data_size; i++) {
        ...
        // some binning modes may now request to check if pixel have been already selected
        // in this case, physical det_id values become negative indicating selection (actual det_id is always positive)
        ...
        // npix_only mode calculates npix distribution only
