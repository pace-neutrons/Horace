...
void bin_io_handler::init(const fileParameters& fpar) {
    ...
    if (!this->h_inout_sqw.is_open()) {
        ...
        mexErrMsgIdAndTxt(this->MEX_ERR_ID.c_str(), err.c_str());
    }
...
bool bin_io_handler::seek_within_bounds(std::streampos position) {
    ...
    if (position < 0 || static_cast<size_t>(position) >= file_size) {
        ...
        buf << "Can not seek to the position: " << position << std::endl;
        mexErrMsgIdAndTxt(this->MEX_ERR_ID.c_str(), buf.str().c_str());
        return false;
    }
    else {
...
void bin_io_handler::write_pix_info(const size_t& num_pixels) {
    ...
    if (this->h_inout_sqw.fail()) {
        std::stringstream buf;
        buf << "Error writing pixel metadata at position: " << pix_info_position << std::endl;
        mexErrMsgIdAndTxt(this->MEX_ERR_ID.c_str(), buf.str().c_str());
