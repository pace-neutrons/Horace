...
!-----------------------------------------------------------------------------------------------------------------------------------
! Rebins histogram data along the y-axis of a 3D dataset.
!
...
! ---------------------------------------
	...
	if ((my < 1) .or. (size(y) /= my+1) .or. (size(e,2) /= my)) then
		call remark ('Check sizes of input arrays (rebin_3d_y_hist)')
		ierr = ERR
		return
	endif
	...
	if ((ny < 1) .or. (size(yout) /= ny+1) .or. (size(eout,2) /= ny)) then
		call remark ('Check sizes of output arrays (rebin_3d_y_hist)')
		ierr = ERR
		return
	endif
