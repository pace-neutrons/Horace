	...
	use maths, only : upper_index, lower_index, single_integrate_3d_x_points
...
!	sout(nb,ny,nz)	real		Output signal
!	eout(nb,ny,nz)	real		Output error bars on signal
!
!-----------------------------------------------------------------------------------------------------------------------------------
...
! ---------------------------------------
	...
	nz = size(s,3)
	if (size(x) /= nx) then
		call remark ('Sizes of x and signal arrays do not correspond (integrate_3d_x_points)')
		ierr = ERR
		return
	endif
	if (size(s) /= size(e)) then
		call remark ('Sizes of signal and error arrays do not correspond (integrate_3d_x_points)')
		ierr = ERR
		return
	endif
	if (nx <= 1) then
		call remark ('Must have at least two data points to perform integration (integrate_3d_x_points)')
		ierr = ERR
		return
	endif
	...
	nb = size(xout)-1
	if (nb<1) then
		call remark ('Size of output integration limits array too small (integrate_3d_x_points)')
		ierr = ERR
		return
	endif
...
! --------------------------
    ...
    do 
        ...
        call single_integrate_3d_x_points(x,s,e,xout(ib),xout(ib+1),ml,mu,sout(ib,:,:),eout(ib,:,:));
