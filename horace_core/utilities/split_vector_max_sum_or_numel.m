...
validateattributes(max_numel, {'numeric'}, {'scalar', 'positive'});
...
n_components   = 1:max_num_chunks ;
if (max_num_chunks == 1) || ...
        (ceil(cumulative_sum(end)/max_chunk_sum) == 1 && max_num_chunks <= max_numel)
...
while end_idx < max_num_chunks
    ...
    end_idx = end_idx + ...
        find(cumulative_sum(start_idx:end) > max_chunk_sum | n_components(1:end -start_idx+1) > max_numel, 1) - 1;
    if isempty(end_idx)
