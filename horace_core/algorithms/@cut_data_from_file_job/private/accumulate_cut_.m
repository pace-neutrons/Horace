function [s, e, npix, pix_range_step, npix_retain,ok, ix] = accumulate_cut_(s, e, npix, pix_range_step, keep_pix,...
    v, proj,pax)
...
if proj.can_mex_cut && use_mex
    [pix_range_step_recent, ok, ix, s, e, npix, npix_retain,mex_success]=...
        proj.accumulate_cut(v,s,e,npix,pax,ignore_nan,ignore_inf,keep_pix,n_threads);
    if npix_retain>0
        pix_range_step =[min(pix_range_step(1,:),pix_range_step_recent(1,:));max(pix_range_step(2,:),pix_range_step_recent(2,:))];  % true range of data
    else
...
if ~use_mex
    [s, e, npix, pix_range_step, npix_retain, ok, ix] = accumulate_cut_matlab_(s, e, npix, pix_range_step, keep_pix,...
        v, proj, pax);
end
