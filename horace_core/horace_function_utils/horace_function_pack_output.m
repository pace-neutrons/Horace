function argout = horace_function_pack_output(w, varargin)
% Pack return arguments for a Horace function that takes object, filename or data source structure input
%
%   >> argout = horace_function_pack_output(w,varargin)
%
% Input:
% ------
%   w               Data source structure created by call to horace_function_parse_input
%                  by the caller
...
if numel(varargin) < w.nargout_req
    error('HORACE:horace_function_pack_output:invalid_argument', ...
          'Insufficient number of return arguments provided');
end
