...
% ---------------------------------------
% One of w1 or w2 must be of the class type, because otherwise the method would
% not have been called. Only if the other is a double array do we need to do
% some parsing on the array
%
if ~isa(w1, 'double')
    inner_section1 = {};
    size1 = size(w1);
...
elseif (len1 > 1 && len2 == 1)
    ...
    error('SQW:binary_op_manager', ...
          ['Array lengths are incompatible.\n'...
           ...
           'number of elements in one of the vectors must be 1.\n' ...
           'Arrays have number of elements ''%i'' & ''%i''.'], len1, len2);
end


%==============================================================================
function [inner_section, outer_dims, mess] = array_split(class_nd, sz)
    % Split double array into inner and outer sections for binary operations on
    % an array of other objects.
    % Deals only with the number of dimensions, but does not check sizes along
    % those dimensions
    %
    %   >> [inner_section,outer_dims] = array_split (class_ndim, class_sz, sz)
    %
    %   class_nd        Number of dimensions of the internal array(s) of object
    %                   (assume class_nd >= 0)
    ...
    %                  each element being an array with the dimensions of
    %                  class_dim, then outer_dims would be the result of the
    %                  intrinsic Matlab SIZE function on that array.

    % The trick as always is to handle the case of Matlab vectors or scalar in
    % a consistent fashion as the definition of the class dimensionality, strip
    % off the inner dimensions, and then leave the excess dimensions (if any)
    % with the correct value as
    ...
    if class_nd <= length(sz)% excess dimensions
        ...
        % Outer dimensions: inner dimensions become 1 and then compress away as
        % many of the inner unity dimensions as possible.
        % Only if length(sz)>2 can we compress; there we must always have at
        % least a length of 2 after compression
        sz(1:class_nd) = 1;
        if length(sz) > 2
            ind = min(find(sz ~= 1));
            if ~isempty(ind)
...
function ok = equal_length_vectors(sz1, sz2)
    % Test if sizes of two array correspond to vectors of same length - either
    ...
    ok = false;
    if (length(sz1) == 2 && (sz1(1) == 1 || sz1(2) == 1)) ...
            && (length(sz2) == 2 && (sz2(1) == 1 || sz2(2) == 1))
