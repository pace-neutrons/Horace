...
if axis==1
    ycen=median(w.data_.p{w.data_.dax(2)});
    zcen=median(w.data_.p{w.data_.dax(3)});
    xmax=max(w.data_.p{w.data_.dax(1)});
    camposvec=[2.*xmax,ycen,zcen];  % Camera position vector
elseif axis==2
    xcen=median(w.data_.p{w.data_.dax(1)});
    zcen=median(w.data_.p{w.data_.dax(3)});
    ymax=max(w.data_.p{w.data_.dax(2)});
    camposvec=[xcen,2.*ymax,zcen];  % Camera position vector
elseif axis==3
    xcen=median(w.data_.p{w.data_.dax(1)});
    ycen=median(w.data_.p{w.data_.dax(2)});
    zmax=max(w.data_.p{w.data_.dax(3)});
