...
for cut_num = 1:prod(sz)
    ...
    if return_cut
        wout{cut_num} = cut_single(obj, targ_proj, targ_ax_block, opt.outfile,opt.proj_given,log_level);
    else
        cut_single(obj, targ_proj, targ_ax_block, opt.outfile,opt.proj_given,log_level);
    end
end
