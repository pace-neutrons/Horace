...
for i=1:args.array_numel
    ...
    elseif ~isempty(args.set_of_fields)
        if numel(args.set_of_fields)>1 && isnumeric(args.set_of_fields{1})...
            ...
            && numel(args.set_of_fields{2}) == 4
            % legacy DnD constructor, used for spinW integration.
            obj_in{i} = DnDBase.spinw_dnd_obj_constructor(args.set_of_fields{:});
            if (obj.NUM_DIMS ~= obj_in{i}.NUM_DIMS)
                error('HORACE:DnDBase:invalid_argument', ...
                    ['%d-dimensional object constructed from array of input ' ...
                    'parameters using legacy spinW constructor differs from' ...
                    ' requested %d-dimensional object'], ...
