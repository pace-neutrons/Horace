function dout = replicate_dnd_(din, dref)
...
if nd_in>nd_ref
    ...
    error('HORACE:sqw:invalid_argument',...
        'Reference dataset must have greater or equal dimensionality of dataset to be replicated');
elseif nd_in==0
    ...
    for i=1:nd_in
        ...
        if ~isempty(ipax)   % axis common
            ...
            if size_in(i)~=size_ref(ipax)
                error('HORACE:sqw:invalid_argument',...
                    'One or more plot axes shared by the input and reference datasets have different length');
            end
        else
            error('HORACE:sqw:invalid_argument',...
                'Input and reference datasets do not share the same plot axes')
        end
    end
    dout = dref;
