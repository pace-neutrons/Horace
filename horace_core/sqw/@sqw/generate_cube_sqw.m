...
out.pix = pix;
samp = IX_sample([2*pi 2*pi 2*pi], [90 90 90]);
expdata = struct( ...
    "filename", 'fake', ...
    "filepath", '/fake', ...
    "efix", 1, ...
...
proj = ortho_proj([1 0 0], [0 1 0],'alatt',2*pi,'angdeg',90,'type','ppr');
req_bin_ranges = {[-shape/2 shape/2],[-shape/2 shape/2],[-shape/2 shape/2],[-shape/2 shape/2]};
ax   = proj.get_proj_axes_block(cell(4,1),req_bin_ranges);

out.data = DnDBase.dnd(ax,proj,npix,npix,npix);
%
out = cut(out, proj, ...
    [minloc 1 maxloc], ...
    [minloc 1 maxloc], ...
