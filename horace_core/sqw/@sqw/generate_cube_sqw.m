...
out.detpar_struct = struct( ...
    ...
    'height', []);
%}
out.detpar_struct=IX_detector_array().get_detpar_representation; % init with default
out.detpar_struct.filename='fake';
out.detpar_struct.filepath='/fake';



samples = out.experiment_info.samples.add(samp);
instruments = out.experiment_info.instruments.add(IX_null_inst());
detector_arrays = IX_detector_array(out.detpar_struct);
