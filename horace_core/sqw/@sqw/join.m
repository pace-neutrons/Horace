...
if ~(hFNM && hFVM)
    error('HORACE:join:invalid_argument', ...
        'sqw objects in input array have non-matching detpar structures')
end
...
wout.pix = PixelDataBase.concatenate(cellfun(@(x) x, pix(run_contributes)));
for i=1:nfiles
