...
for i=1:numel(xlist)
    ...
    if ~isempty(tmp)
        ...
        error('HORACE:invalid_argument',...
            'Unrecognised coordinate name: "%s"',xlist{i});
...
if any(logical(ind(noff+nd:noff+3)))
    error('HORACE:invalid_argument',...
        'Coordinate name incompatible with dimensionality of sqw object (=%d)',nd);
end

% Can inly perform operation for sqw-type object
if ~isa(w,'sqw')
    error('HORACE:invalid_argument',...
        'Function only has meaning for an sqw-type object, not a dnd-type sqw object');
end
...
if present.h||present.k||present.l||present.Q
    ...
    uhkl=header_ave.u_to_rlu(1:3,1:3)*w.data.pix.q_coordinates+repmat(header_ave.uoffset(1:3),[1,npixtot]);
    if present.Q
        % Get |Q|
...
if present.d1||present.d2||present.d3||present.d4
    ...
    uproj=U*w.data.pix.q_coordinates-repmat(T,[1,npixtot]);        % pixel Q coordinates now in projection axes
