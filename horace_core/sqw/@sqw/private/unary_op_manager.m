...
for i=1:numel(w1)
    if is_sqw_type(w1(i))
        result = unary_op(sigvar(w1(i).data.pix.signal, w1(i).data.pix.variance));   % Apply operation to pixel data
        w(i).data.pix.signal = result.s;
        w(i).data.pix.variance = result.e;
