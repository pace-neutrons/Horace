...
elseif narg >= 1 && narg <= 4
    ...
    if ~isnumeric(alatt) || numel(alatt) ~= 3 || any(alatt <= 0)
        ok = false;
        mess = 'Check new lattice parameters [a,b,c] are all greater than zero';
        return
    end
    if narg >= 2
        ...
        if ~isnumeric(angdeg) || numel(angdeg) ~= 3 || any(angdeg <= 0)
            ok = false;
            mess = 'Check new lattice angles [alf,bet,gam] are all greater than zero';
            return
        end
    else
        angdeg = angdeg0;
    ...
    elseif narg == 3
        ...
        if ~(isnumeric(rotmat) && numel(size(rotmat) == 2) && all(size(rotmat) == [3,3]) && abs(det(rotmat) - 1) < 1e-12)
            ok=false;
            mess='Check rotation matrix is orthogonal with determinant=+1';
            return
        end
    ...
    elseif narg == 4
        ...
        if (~isnumeric(u) || numel(u) ~= 3 || all(abs(u) <= 1e-12)) || (~isnumeric(v) || numel(v) ~= 3 || all(abs(v) <= 1e-12))
            ok=false;
            mess='Check new orientation vectors u and v';
            return
        end
        u0 = header_ave.cu;
        v0 = header_ave.cv;
