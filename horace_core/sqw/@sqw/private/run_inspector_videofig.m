function [fig_handle, axes_handle, scroll_bar_handles, scroll_func] = ...
    run_inspector_videofig(num_frames, redraw_func, redraw_args, play_fps, big_scroll, ...
    key_func, varargin)
...
%   This can be used not only to play and analyse standard videos, but it
...
if nargin < 4 || isempty(play_fps), play_fps = 5; end  %play speed (frames per second)
...
%RAE hack:
scroll_handle = patch([0 1 1 0] * scroll_bar_width, [0 0 1 1], [0 0 0], ...
    'Parent',scroll_axes_handle, 'EdgeColor','none', 'ButtonDownFcn', @on_click);
...
%axes_handle = axes('Position',[0 0.03 1 0.97]);%original values
axes_handle = axes('Position',[0.13 0.13 0.74 0.74]);%RAE messing

%return handles
scroll_bar_handles = [scroll_axes_handle; scroll_handle];
scroll_func = @scroll;

%set figure background to be white by default:
set(gcf,'Color',[1,1,1]);
...
my_char={'Enter (Return) -- play/pause video (5 frames-per-second default)',...
    ...
    'Home/end -- go to first/last frame of video'};
msgbox(my_char,'Horace Animation Info');
