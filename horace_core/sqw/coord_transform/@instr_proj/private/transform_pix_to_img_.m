function pix  = transform_pix_to_img_(obj,run_data,varargin)
...
[run_data.S,obj.ERR,run_data.det_par,non_masked]  = run_data.rm_masked(ignore_nan,ignore_inf);
...
pix = calc_projections_(obj, detdcn,qspec,proj_mode);



function pix = calc_projections_(obj, detdcn,qspec,proj_mode)
...
% disable filebased pixels -- this projection should not be filebased
pix_page_size = 1.e+34;
...
if use_mex
    if ~isempty(qspec) % why is this?
        ...
        try
            ...
            if proj_mode==2
                pix = PixelData(pix,pix_page_size,false);
...
if ~use_mex
    ...
    pix = PixelData([ucoords;run_id;detector_idx;energy_idx;sig_var],pix_page_size,false);
    pix.set_range(pix_range);    
end
