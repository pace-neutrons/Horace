...
bsize = cur_axes_block.nbins_as_size();
bsize = bsize(1:3)+1 ; % the size of q_block; +1 to size(npix) accounting for left axis range
...
if dEgrid(end)<=bin_range(2,4) && bin_range(2,4)<=dEgrid(end) % Top boundary of the energy grid goes
    e_inside(end) = false;     % to last bin, so not included in stride.
    e_inside(end-1) = true;
end
...
q_block_size= prod(bsize-1); % the size of q_block in memory for each dE bin
