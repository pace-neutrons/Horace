function [ pbin_out, ndims] = calc_pbins(proj, pix_range_in, pbin, pin, en)
% Check binning descriptors are valid, and resolve multiple integration axes
% using limits and bin widths from the input data.
%
%   >> [ok, mess, pbin_out, ndims] = cut_sqw_calc_pbins (pix_range, proj, pbin, pin, en)
%
% Input:
% ------
%   pix_range_in   [2x4] array of range of data along the input projection axes (elements must all be finite)
%
...
[~, ~, pax, p] = proj.calc_ubins (pix_range_in,pbin_tmp, pin, en);
ndims = numel(p);
multi = (n_pbin==4);
if any(multi)
