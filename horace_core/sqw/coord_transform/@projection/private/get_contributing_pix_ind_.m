...
if ebin==1 && trans_elo==0   % Catch special (and common) case of energy being an integration axis to save calculations
    indx=[bsxfun(@minus,v.coordinates(1:3,:)',trans_bott_left')*rot_ustep',v.dE'];  % nx4 matrix
else
    indx=[bsxfun(@minus,v.coordinates(1:3,:)',trans_bott_left')*rot_ustep',(v.dE'-trans_elo)*(1/ebin)];  % nx4 matrix
end
