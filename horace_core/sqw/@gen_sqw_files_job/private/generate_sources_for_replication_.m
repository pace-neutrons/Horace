function [spe_files_out,duplicated_fnames] = generate_sources_for_replication_(spe_files,n_workers)
...
if any(is_empty)
    % empty files are not converted to tmp using
...
if numel(unique_fnames) == numel(spe_files)
    ...
    spe_files_out = do_recover_empty(spe_files,is_empty,recover_empty);
    return;
end
...
duplicated_fnames = cell(1,numel(spe_files)-n_unique);
copies_list       = containers.Map(unique_fnames,num2cell(zeros(1,n_unique)));
last_copies       = unique_fnames;
%
n_spe = 0;
n_duplicate = 1;
for i=1:n_workers
    ...
    for j=1:numel(worker_file_idx)
        ...
        if any(is_wkr_unique) % this is unique file_idx taken first time
            ...
            if n_copies == 0
                ...
                last_copies{f_idx} = file_name;
            end
        else
            file_name = last_copies{f_idx};
        end
        n_spe            = n_spe+1;
