function [wout, fitdata, ok, mess] = fit_legacy_sqw_sqw(win, varargin)
% Fit a model for S(Q,w) to a d1d object, with an optional background
% function that is also a model for S(Q,w).
%
...
% -------------------------------------------------------
%   >> [wout, fitdata] = fit_legacy_sqw_sqw (w, func, pin)
%   >> [wout, fitdata] = fit_legacy_sqw_sqw (w, func, pin, pfree)
%   >> [wout, fitdata] = fit_legacy_sqw_sqw (w, func, pin, pfree, pbind)
%
...
% --------------------------------------------------------
%   >> [wout, fitdata] = fit_legacy_sqw_sqw (..., bkdfunc, bpin)
%   >> [wout, fitdata] = fit_legacy_sqw_sqw (..., bkdfunc, bpin, bpfree)
%   >> [wout, fitdata] = fit_legacy_sqw_sqw (..., bkdfunc, bpin, bpfree, bpbind)
%
%
...
%
%   >> [wout, fitdata] = fit_legacy_sqw_sqw (..., keyword, value, ...)
%
...
%
%   EXAMPLES:
%   >> [wout, fitdata] = fit_legacy_sqw_sqw(...,'keep',[0.4,1.8],'list',2)
%
%   >> [wout, fitdata] = fit_legacy_sqw_sqw(...,'select')
%
...
%
%   >> [wout, fitdata, ok, mess] = fit_legacy_sqw_sqw (...)
%
%
%------------------------------------------------------------------------------
% Description in full
...
% Input:
% ======
%   w       d1d object or array of d1d objects to be fitted
%
...
% Output:
% =======
%   wout    d1d object or array of d1d objects evaluated at the
...
% =========
%
% Fit a spin waves to a d1d object, allowing only intensity and coupling
% constant to vary:
...
%   first_line = {'% Fit a model for S(Q,w) to a d1d object, with an optional background',...
%                 '% function that is also a model for S(Q,w).'}
%   main = false;
%   method = true;
%   synonymous = false;
...
if nargout<3
    [wout,fitdata]=fit_sqw_sqw(sqw_old(win), varargin{:});  % forces failure if there is an error, as is the convention for fit when no ok argument
else
    [wout,fitdata,ok,mess]=fit_sqw_sqw(sqw_old(win), varargin{:});
end
wout=dnd(wout);
