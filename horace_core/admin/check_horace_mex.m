...
for i=1:numel(functions_name_list)
    try
        ...
        if contains(functions_name_list{i},'combine_sqw')
            can_use_mex_4_combine=false;
        elseif contains(functions_name_list{i},'hdf_mex_reader')
            % TODO! Temporary disabled check for failure as hdf_mex_reader
