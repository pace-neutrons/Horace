function [wout, fitdata, ok, mess] = multifit_legacy_func(win, varargin)
...
% -------------------------------------------------------
%   >> [wout, fitdata] = multifit_legacy_func (w, func, pin)
%   >> [wout, fitdata] = multifit_legacy_func (w, func, pin, pfree)
%   >> [wout, fitdata] = multifit_legacy_func (w, func, pin, pfree, pbind)
%
...
% --------------------------------------------------------
%   >> [wout, fitdata] = multifit_legacy_func (..., bkdfunc, bpin)
%   >> [wout, fitdata] = multifit_legacy_func (..., bkdfunc, bpin, bpfree)
%   >> [wout, fitdata] = multifit_legacy_func (..., bkdfunc, bpin, bpfree, bpbind)
%
%
...
%
%   >> [wout, fitdata] = multifit_legacy_func (..., keyword, value, ...)
%
...
%
%   EXAMPLES:
%   >> [wout, fitdata] = multifit_legacy_func(...,'keep',[0.4,1.8],'list',2)
%
%   >> [wout, fitdata] = multifit_legacy_func(...,'select')
%
...
%
%   >> [wout, fitdata, ok, mess] = multifit_legacy_func (...)
%
%
%------------------------------------------------------------------------------
% Description in full
...
% Input:
% ======
%   w       d3d object or array of d3d objects to be fitted
%
...
%           the form:
%               ycalc = my_function (x1,x2,x3,p)
%
%             or, more generally:
%               ycalc = my_function (x1,x2,x3,p,c1,c2,...)
%
%             where
%               - x1,x2,x3  Arrays of x values along first, second, third
%                          dimensions
...
% Output:
% =======
%   wout    d3d object or array of d3d objects evaluated at the
...
%   >> [yfit,fitpar]=multifit_legacy_func(w,@gauss,pin,@linear_bg,bg)
%
% Remove a portion of the data, and give copious output during the fitting
%
%   >> [yfit,fitpar]=multifit_legacy_func(w,@gauss,pin,@linear_bg,bg,'remove',[12,14],'list',2)
%
% Fix the position and constrain the width to be a constant multiple of
% the constant part of the linear background:
%
%   >> [yfit,fitpar]=multifit_legacy_func(w,@gauss,pin,[1,0,1],{3,1,1},@linear_bg,bg)
...
%   obj_name = 'd3d'
%
%   doc_forefunc = fullfile(sqw_doc,'doc_fitfunc_d3d_simple.m')
%   doc_backfunc = fullfile(sqw_doc,'doc_fitfunc_d3d_simple.m')
%
