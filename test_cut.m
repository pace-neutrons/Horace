classdef test_cut < TestCaseWithSave & common_state_holder
    ...
    properties
        ...
        sqw_4d;
        test_par_file = 'MAP11014.nxspe'; % located in common data
        sqw_4D_artificial
    end
    ...
    methods
        ...
        function obj = test_cut(varargin)
            ...
            par_file = fullfile(fileparts(this_dir),...
                'common_data',obj.test_par_file);
            en = -5:1:80;
            efix = 85;
            alatt = [2.83,2.83,2.83];
            angdeg = [90,90,90];            
            wtmp = fake_sqw(en,par_file,'',efix,1,...
            alatt,angdeg,[1,0,0],[0,1,0],5,0,0,0,0,[50,50,50,50]);
        ...
        function test_take_simple1Dcut_from_newly_generated_sqw_object(obj)
            sqw_obj = obj.sqw_4D_artificial;
            pr = struct('u',[1,0,0],'v',[0,1,0]);
            cut1D_ng = cut(sqw_obj,pr,[-1,0.01,1],[-0.1,0.1],[-0.1,0.1],[-5,5]);
