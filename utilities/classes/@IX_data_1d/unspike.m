function wout = unspike (w,varargin)
% Remove points deemed to be spikes, and replace with values interpolated between good points
%
%   >> wout = unspike (w)
%   >> wout = unspike (w,smin,smax,fac,sigmafac)    % set to '' or [] to use default value of any argument
%
% Input:
% ------
%   w       Input IX_dataset_1d or array of IX_dataset_1d
...
% Output:
% -------
%   wout    IX_dataset_1d or array of IX_dataset_1d with spikes removed
...
for i=1:numel(w)
    if numel(w(i).signal)>0    % if empty data, dont do anything
        if ishistogram(w(i))
            ...
            [wout(i).signal,wout(i).error]=unspike_xye(xc,w(i).signal,w(i).error,varargin{:});
        else
            [wout(i).signal,wout(i).error]=unspike_xye(w(i).x,w(i).signal,w(i).error,varargin{:});
        end
    end
end
