function w = binary_op_manager_(w1, w2, binary_op,classname)
...
elseif ((isequal(size1,size2) || equal_length_vectors(size1,size2)) && len1>1)   % same length>1 and same array size
    ...
    w = repmat(w,size_w);
    for i=1:len1
        w(i) = binary_op_manager_single_(w1(inner_section1{:},i), w2(inner_section2{:},i), binary_op,classname);
    end
...
elseif (len1==1 && len2>1)
    ...
    for i=1:len2
        w(i) = binary_op_manager_single_(w1, w2(inner_section2{:},i), binary_op,classname);
    end
...
elseif (len1>1 && len2==1)
    ...
    for i=1:len1
        w(i) = binary_op_manager_single_(w1(inner_section1{:},i), w2, binary_op,classname);
    end
...
%
%   inner_section   Cell array of class_nd repeats of ':' for array sectioning
