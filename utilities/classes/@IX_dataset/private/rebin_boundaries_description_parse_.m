function [ok,xbounds,any_lim_inf,is_descriptor,any_dx_zero,mess]=rebin_boundaries_description_parse_(nax,opt,varargin)
...
if nax==1
    if ~(numel(varargin)==1 && (isempty(varargin{1}) || (isnumeric(varargin{1})&&isvector(varargin{1}))))
        ...
        [ok,xbounds{1},any_lim_inf,is_descriptor,any_dx_zero,mess]=rebin_boundaries_description_parse_single_(opt,xvals);
...
if nax==numel(varargin)
    ...
    any_dx_zero=false(1,nax);
    for i=1:nax
        [ok,xbounds{i},any_lim_inf(i),is_descriptor(i),any_dx_zero(i),mess]=rebin_boundaries_description_parse_single_(opt,varargin{i});
