function w2 = IX_dataset_2d(w1,varargin)
...
nw=numel(w1);
if isempty(w1)
    error('Input IX_dataset_1d is empty')
end
...
if narg>=1 && ~isempty(varargin{1})
    ...
    elseif iscellnum(y)
        ...
        error('Check input arguments - y values must be array or cell array of arrays')
    end
else
    y=[];
end
...
if narg>=3 && ~isempty(varargin{3})
    if islognum(varargin{3})
        ...
        error('Check bindata argument - must be logical scalar or array (or numeric array of 0 and 1)')
    end
else
    y_dist=false;
    ny_dist=1;
    ...
    if (ny==1||ny==nel) || (ny_axis==1||ny_axis==nel) || (ny_dist==1||ny_dist==nel) || (isempty(bindata)||nbindata==1||nbindata==nel)
        ...
        if isempty(bindata)     % fill according to unambiguous determination from number of IX_dataset_1d and contenst of y
            ...
            elseif sum(nely)==nw+nel
                ...
                error('Number of y values inconsistent with number of number of elements in input IX_dataset_1d object')
            end
        elseif nbindata~=nel
...
w2=IX_dataset_1d_to_2d(w1,y,y_axis,y_dist,nelyc);

end

%-------------------------------------------------------------------------------------------------
function w2=IX_dataset_1d_to_2d(w1,y,y_axis,y_dist,nelyc)
...
for i=1:numel(nelyc)
    w2(iw2beg(i):iw2end(i)) = IX_dataset_1d_to_2d_chunk(w1(iw1beg(i):iw1end(i)),...
...
%-------------------------------------------------------------------------------------------------
function w2=IX_dataset_1d_to_2d_chunk(w1,y,y_axis,y_dist,n,nlo,nhi)
% Repack an array of IX_dataset_1d into an array of IX_dataset_2d according
...
for i=1:n
    ...
    w2(i)=IX_dataset_2d(w1(nlo(i)).title,signal,error,w1(nlo(i)).s_axis,w1(nlo(i)).x,...
        w1(nlo(i)).x_axis,w1(nlo(i)).x_distribution,y(nlo(i):nhi(i)+nbin),y_axis,y_dist);
end
