function wout = simple_rebin_z(win, varargin)
% Rebin IX_dataset_3d along z axis using reference 1D algorithm
%
%   >> wout = simple_rebin_z(win, ylo, yhi)       % keep data between ylo and yhi, retaining existing bins
%	>> wout = simple_rebin_z(win, ylo, dy, yhi)   % rebin from ylo to yhi in intervals of dy
%   >> wout = simple_rebin_z(win,wref)            % rebin win with the bin boundaries of wref (a 1D dataset)
%   >> wout = simple_rebin_z(..., 'int')          % trapezoidal integration if point data
%
