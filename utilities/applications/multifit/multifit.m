...
for i=1:numel(args)
    if isa(args{i},'function_handle')
        if isscalar(args{i})
            ...
            error('Fitting function handle must be a scalar')
        end
        break
    end
end
if isempty(iarg_global_func)
    error('Must provide handle to fitting function')
end
        ...
        for i=1:numel(w)
            if isstruct(w{i})
                if isscalar(w{i})
                    ...
                    if ~ok
                        error(['Data cell array element ',arraystr(size(w),i),' is a structure : ',mess]);
                    end
                else
                    error(['Data cell array element ',arraystr(size(w),i),' invalid: is an array of structures']);
                end
            else
                if ~parsefunc   % don't check validity of method if just testing the parsing
                    ...
                    if ~ismethod(w{i},tmp.function)
                        error(['Data cell array element ',arraystr(size(w),i),': fit function is not a method of this object']);
                    end
                end
            end
        end
    elseif isstruct(w)
        ...
        if ~parsefunc   % don't check validity of method if just testing the parsing
            ...
            if ~ismethod(w,tmp.function)
                error('Data object: fit function is not a method of this object');
            end
        end
    end
elseif iarg_global_func==4
    ...
    error('Syntax of data argument(s) is invalid')
end
        ...
        if is_function_handles(args{i})
            ...
            if isa(args{i},'function_handle')
                ...
                elseif isscalar(args{i})
                    ...
                    error('Background function handle parameter must be scalar or have same size as data array')
                end
            end
            break
        end
...
if nglobal_args>=1
    ...
    if ~ok
        error('Check that the fitting parameter list is valid')
    end
else
    error('Must give fitting function parameters')
end
...
if bkd
    if nbkd_args>=1
        ...
        error('Must give background function(s) parameters')
    end
else
    nbp=zeros(size(w));
    bpin=cell(size(w));
...
if parsefunc
    varargout{1}=iarg_global_func;
    varargout{2}=func;
    varargout{3}=pin;
    if bkd
        varargout{4}=iarg_bkd_func;
    else
        varargout{4}=[];
    end
    varargout{5}=bkdfunc;
...
elseif nglobal_args==3  % must be pfree followed by pbind
    ...
    error('Too many optional arguments for fitting function')
end
...
if ~isempty(options.keep)
    ...
    if ~(isscalar(xkeep) || isequal(size(w),size(xkeep)))
        error('''keep'' option must provide a single entity defining keep ranges, or a cell array of entities with same size as data source ')
    end
...
if ~isempty(options.remove)
    ...
    if ~(isscalar(xremove) || isequal(size(w),size(xremove)))
        error('''remove'' option must provide a single entity defining remove ranges, or a cell array of entities with same size as data source ')
    end
...
if ~isempty(options.mask)
    ...
    if ~(isscalar(msk) || isequal(size(w),size(msk)))
        error('''mask'' option must provide a single mask, or a cell array of masks with same size as data source ')
    end
...
for i=1:numel(w)
    ...
    if xye(i)    % xye triple
        ...
        if ok && ~isempty(mess), display_mess(data_id,mess), elseif ~ok, error([data_id,mess]), end   % display warning messages
