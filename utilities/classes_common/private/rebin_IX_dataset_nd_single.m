function [wout,ok_rebin,mess] = rebin_IX_dataset_nd_single(win,iax,xbounds,true_values,integrate_data,point_integration,use_mex,force_mex)
% Rebin dataset. Assumes that have already checked validity of input data.
%
%   >> [wout,ok_rebin,mess] = rebin_IX_dataset_nd_single(win,iax,xbounds,true_values,integrate_data,point_integration,use_mex,force_mex)
%
% Input:
% -------
...
ax=axis(win,iax(1));
[wout_x{1},wout_s,wout_e,ok_rebin,mess] = rebin_one_axis(ndim,iax(1),ax.values,win.signal,win.error,ax.distribution,xbounds{1},true_values(1),...
                                                integrate_data,point_integration(1),use_mex,force_mex);
if ~ok_rebin, wout=IX_dataset_nd(ndim); return, end

for i=2:nrebin
    ax=axis(win,iax(i));
    [wout_x{i},wout_s,wout_e,ok_rebin,mess] = rebin_one_axis(ndim,iax(i),ax.values,wout_s,wout_e,ax.distribution,xbounds{i},true_values(i),...
...
%============================================================================================================
function [wout_x,wout_s,wout_e,ok_rebin,mess] = rebin_one_axis(ndim,iax,win_x,win_s,win_e,win_xdist,xbounds,true_values,...
    integrate_data,point_integration,use_mex,force_mex)
% Rebin dataset. Assumes that have already checked validity of input data.
%
%   >> [wout_x,wout_s,wout_e,ok_rebin,mess] = rebin_one_axis(ndim,iax,win_x,win_s,win_e,win_xdist,xbounds,true_values,...
%                                               integrate_data,point_integration,use_mex,force_mex)
%
% Input:
% -------
...
% Histogram data
if nx~=sz_full(iax)
    % Catch case when no data along integration axis, then cannot integrate data. 
    if numel(win_x)==0
...
% Point data
    ...
    if ~point_integration
        ...
        if isempty(ind)     % there are no points in the new bins - no need to perform any more calculations
            sz_full(iax)=0;
            wout_x=zeros(1,0);
            wout_s=zeros(sz_full);
            wout_e=zeros(sz_full);
            ...
            % Catch case that there are already no points along one of the other axes
            if any(sz==0)
        ...
        % fact that the value of the data points is undefined. However, we should assign on the basis of wht we would do
