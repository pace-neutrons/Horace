function [sout,eout] = rebin_3d_z_hist (x, s, e, xout, use_mex, force_mex)
% Rebins histogram data along axis iax=3 of an IX_dataset_nd with dimensionality ndim=3.
%
%   >> [sout,eout] = rebin_3d_z_hist (x, s, e, xout, use_mex, force_mex)
%
...
if use_mex
    try
        [sout,eout] = rebin_3d_z_hist_mex (x, s, e, xout);
    catch
        if ~force_mex
...
if ~use_mex
    [sout,eout] = rebin_3d_z_hist_matlab (x, s, e, xout);
end
