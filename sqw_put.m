function [mess,position,npixtot,type] = sqw_put (outfile,main_header,header,detpar,data,varargin)
% Write an sqw data structure to file
%
%   >> [mess, position, npixtot, type] = sqw_put (outfile, main_header, header, detpar, data)
%   >> [mess, position, npixtot, type] = sqw_put (outfile, main_header, header, detpar, data, '-nopix')
%   >> [mess, position, npixtot, type] = sqw_put (outfile, main_header, header, detpar, data, '-pix')
%   >> [mess, position, npixtot, type] = sqw_put (outfile, main_header, header, detpar, data, '-pix',...
...
% Write sqw_type
mess = put_sqw_file_type (fout, sqw_type);
if ~isempty(mess); if close_file; fclose(fout); end; return; end
