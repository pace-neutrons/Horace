...
% Draw a surface plot of a 2D sqw object or array of sqw objects, with colour scale from a second source
%
...
if numel(varargin)>0 && (isa(varargin{1},class(w))||isa(varargin{1},'d2d')||(isnumeric(varargin{1})&&rem(numel(varargin),2)==1))
    ...
    % Perform plot
    [figureHandle_, axesHandle_, plotHandle_] = ds2(IX_dataset_2d(w), wc, varargin{2:end}, 'name', name_surface);
    if ~ok
        error(mess)
    end
else
    [figureHandle_, axesHandle_, plotHandle_] = ds2(IX_dataset_2d(w), varargin{:}, 'name', name_surface);
    if ~ok
        error(mess)
    end
end
