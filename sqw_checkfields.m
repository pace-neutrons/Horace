function [ok, mess, type, dout] = sqw_checkfields (d, type_in)
% Check fields of a structure match those for a valid sqw object
%
%   >> [ok, mess, type, dout] = sqw_checkfields (d, type_in)
%
% Input:
% ------
...
% sqw_check_* functions for more details.
...
elseif ~(isequal(type_in,'a')||isequal(type_in,'b+'))
    error('Invalid argument type_in to sqw_checkfields - logic problem in Horace')
end
...
% --------------------------
[ok,mess] = sqw_check_main_header(d.main_header);
...
% Check header(s)
% ---------------------
[ok,mess] = sqw_check_header(d.header);
...
% -----------------------
[ok,mess] = sqw_check_detpar(d.detpar);
...
% Check data block
% ---------------------
[ok,type,mess] = sqw_check_data(d.data,tmp_type);
