...
if nargout<3
    [wout,fitdata]=fit_sqw_sqw(sqw(win), varargin{:});  % forces failure if there is an error, as is the convention for fit when no ok argument
else
    [wout,fitdata,ok,mess]=fit_sqw_sqw(sqw(win), varargin{:});
end
wout=dnd(wout);
