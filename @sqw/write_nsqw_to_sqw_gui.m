function write_nsqw_to_sqw_gui (dummy, infiles, outfile,hObject,handles)
...
for i=1:nfiles
    ...
    %
    drawnow;
    %check that the stop button has not been pressed:
    ...
    if status==0
        ...
        disp('You must run this again to get data');
        disp(' ');
        disp(' ');
        return;
    end
...
run_label='fileno';
mess = put_sqw_gui (outfile, main_header_combined, header, det, sqw_data, hObject,handles,'-pix', infiles, pos_npixstart, pos_pixstart, run_label);
