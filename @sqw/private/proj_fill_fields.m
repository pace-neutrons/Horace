...
if isfield(proj_in,'uoffset')
    if isa(proj_in.uoffset,'double') && isvector(proj_in.uoffset) && (length(proj_in.uoffset)==3||length(proj_in.uoffset)==4)
        ...
        proj.uoffset = zeros(4,1);          % default value column vector
        proj.uoffset(1:n)= proj_in.uoffset; % fill with values from input (energy offset will by default be zero)
    else
        proj=[];
...
if isfield(proj_in,'lab')
    ...
    if iscellstr(proj_in.lab)
        proj.lab=proj_in.lab(:)';   % ensure row cell array
    else
        proj=[];
