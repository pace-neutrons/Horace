function [mess,position,npixtot,type] = put_sqw_data_gui (fid, data, hObject, handles, opt, infiles, npixstart,...
    pixstart, run_label)
...
elseif strcmpi(type_in,'a-')
    ...
    elseif ischar(opt) && strcmpi(opt,'-nopix')
        ...
        pix_from_file = false;
    elseif ischar(opt) && strcmpi(opt,'-pix')
        if nargin==9    % pixel file information
            pix_from_struct = false;
            pix_from_file = true;
        else
            mess = 'Check number of input arguments (type of input data is ''a-'')';
...
if strcmpi(type_in,'a')||strcmpi(type_in,'a-')
    ...
    elseif pix_from_file
        ...
        position.pix=ftell(fid);
        type='a';
        mess = put_sqw_data_pix_from_file_gui (fid, infiles, npixstart, pixstart, npix_cumsum, run_label, hObject, handles);
    else
        type='a-';
    end
end
