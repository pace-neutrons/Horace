function [tmp_file,grid_size,urange] = gen_sqw_gui (dummy, spe_file, par_file, sqw_file, efix, emode, alatt, angdeg,...
                                                u, v, psi, omega, dpsi, gl, gs, hObject,handles,grid_size_in, urange_in)
...
%   handles         Structure array of callbacks for GUI
...
% Modification of gen_sqw for use with Horace GUI - R.A. Ewings 27/11/2008
...
for i=1:nfiles
    ...
    evalstring='get(handles.Run_pushbutton,''UserData'');';
    status=evalin('caller',evalstring);
    if status==0
        disp(' ');
        disp(' ');
        disp('----STOPPING SQW file generation----');
        disp('-------Operation incomplete---------');
        disp('------SQW file not generated--------');
        disp(' ');
        disp(' ');
        return;
    end
...
write_nsqw_to_sqw_gui (tmp_file, sqw_file,hObject,handles);
disp('--------------------------------------------------------------------------------')
