...
void mexFunction(int nlhs, mxArray *plhs[ ],int nrhs, const mxArray *prhs[ ])
{
  ...
  double * const pPixelSorted = (double *)mxGetPr(prhs[Pixels_Sorted]);

  mwSize  *const ppInd   = (mwSize *)mxCalloc(nPixDataCols, sizeof(mwSize)); //working array of indexes for transformed pixels
  if(!ppInd){
...
void sort_pixels_by_bins(double const *const pPixelData,mwSize nDataRows,mwSize nDataCols,double const *const pCellInd,
	...
	for(i=1;i<distribution_size;i++){   // calculate the ranges of the cell arrays
		ppInd[i]=ppInd[i-1]+(mwSize)pCellDens[i-1];
	}; 
