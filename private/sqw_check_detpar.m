function [ok, mess] = sqw_check_detpar (det, field_names_only)
% Check that the fields in the detector parameters are OK
%
%   >> [ok, mess] = sqw_check_detpar (det)
%   >> [ok, mess] = sqw_check_detpar (det, field_names_only)
%
% Input:
% ------
...
elseif ~(isnumeric(field_names_only)||islogical(field_names_only))
    error('Invalid argument field_names_only to sqw_check_detpar - logic problem in Horace')
end
