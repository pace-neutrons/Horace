function [ok, mess] = sqw_check_main_header (main_header, field_names_only)
% Check that the fields in the main header are OK
%
%   >> [ok, mess] = sqw_check_main_header (main_header)
%   >> [ok, mess] = sqw_check_main_header (main_header, field_names_only)
%
% Input:
% ------
...
elseif ~(isnumeric(field_names_only)||islogical(field_names_only))
    error('Invalid argument field_names_only to sqw_check_main_header - logic problem in Horace')
end
