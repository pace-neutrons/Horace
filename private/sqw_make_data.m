function [data,mess] = sqw_make_data (varargin)
% Make a valid detpar field
% Create a valid structure for an sqw object
%
%   >> [data,mess] = sqw_make_data (u0,u1,p1,u2,p2,...,un,pn)
%   >> [data,mess] = sqw_make_data (u0,u1,p1,u2,p2,...,un-1,pn-1,pn)
%   >> [data,mess] = sqw_make_data (lattice,u0,...)
%   >> [data,mess] = sqw_make_data (ndim)
%
% Input:
% -------
...
if narg==0 || (narg==1 && isscalar(varargin{1}) && isnumeric(varargin{1}))
    % Call of form: sqw_make_data or sqw_make_data(ndim)
    if narg==0
        ndim=0;
    else
        ndim=varargin{1};
...
elseif narg>=1
    ...
    ndim=ceil(narg/2);  % don't forget that can omit argument: un
    if ndim<0 || ndim>4, mess='ERROR: Check number of arguments'; return; end;    % Incorrect number of u's and p's
    implicit_en_axis=2*ndim-narg;   % =1 if the last axis is implicitly energy, =0 otherwise
    ...
    [rlu_to_ustep, u_to_rlu, ulen, mess] = rlu_to_ustep_matrix (latt(1:3), latt(4:6), u_to_rlu(1:3,1)', u_to_rlu(1:3,2)', [1,1,1], 'rrr');
