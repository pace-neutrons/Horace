function [ok, type, mess] = sqw_check_data (data, type_in, field_names_only)
% Check that the fields in the data are OK
%
%   >> [ok, mess] = sqw_check_data (data)
%   >> [ok, mess] = sqw_check_data (data, type_in)
%   >> [ok, mess] = sqw_check_data (data, type_in, fields_names_only)
%
% Input:
% ------
...
elseif ~(isequal(type_in,'a')||isequal(type_in,'b+'))
    error('Invalid argument type_in to sqw_check_data - logic problem in Horace')
end
...
elseif ~(isnumeric(field_names_only)||islogical(field_names_only))
    error('Invalid argument field_names_only to sqw_check_data - logic problem in Horace')
end
