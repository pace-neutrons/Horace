...
Horace parallel job tasks are controlled and communicate with each other using this interface, which makes them 
of the underlying communication method.
...
This restricts the use of file-based framework to the corresponding part of Horace algorithms and for initial job submission.
...
| `task_outputs`          | P   | The property, containing the result of `do_job` operation to distribute to another nodes or to transfer to the head node(node-1) for if these outputs are defined.                                              |
...
| `check_availability`            | M     | the method, returning true if cluster of given type is available on given machine. E.g., check_availability  for **ClusterParpoolWrapper** will return false if parallel computing toolbox is not installed on the given machine.                                                                       |
...
| `throw_on_interrupts`       | P     | The property defines framework behaviour in case when interrupt message (cancelled or failed) received through the network. Normally this means that all processing would be completed and worker shut-down so exception would be thrown. When the framework is gathering information on exceptions to report issue to user, interrupt messages are received to be processed by framework to return the diagnostics to users. The property in this case is set to false. |
...
[^1]: The developer should expect that errors may happen during a worker execution, and a *Fail* or *Canceled* message can be sent from the client instead of the regular exchange message.
...
Such a message, received by a worker from other worker, blocks all further communication with this worker.
...
```
to adapt its behaviour to a parallel environment.
...
This exception, caught within its parallel worker, finishes appropriate task.
