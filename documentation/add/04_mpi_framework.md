...
| Method or Property| M/P |Description |
| :--- | :--- | :--- | 
|`job_id`  | P  | The string, providing unique identifier(name) for the running cluster and the job running on this cluster. When cluster is running, it is the same name as *job\_id* in *JobDispatcher*. Ths name is also propagated to *iMessagesFramework* below.|
