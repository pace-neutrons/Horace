...
Before the properties are set within the serializable class constructor one by one, the *serializable* code sets the protected property **check_combo_arg_** to false, so the check for each interdependent property is not occurring. The code then sets the property **check_combo_arg_** to true and runs the check method: *check_combo_arg* after all interdependent properties have been set, throwing if **check_combo_arg** returns **false**. As the result, user can not change interdependent properties in a wrong way and needs to use constructor or serializable interface to set defined class with interdependent properties.
...
```
where *positional_val1, positional_val2...* are the values, one would set up to the public properties of the *serializable* interface in the order, these properties names are returned by *saveableFields* method. The `keys` then would be the names of these properties, followed by the appropriate values.
