...
\begin{tikzpicture}[x={(-5mm,-3.85mm)},z={(0,1cm)},y={(1cm,-.3cm)}]
  ...
  \draw [->] (0,0) -- (0,4,0) node [above,xshift=0.cm,yshift=0.3cm] {$\vec{e_y} = \frac{\vec{u} \times \vec{v}}{|u|\cdot |v|}$};
  \draw [->] (0,0) -- (0,0,4.2) node [right,yshift=-0.2cm] {$\vec{u}$};

  \draw [->] (0,0) -- (6,0,0) node [right,xshift=0.1cm,yshift=-0.1cm] {$\vec{e_y} \times \vec{u}$};
