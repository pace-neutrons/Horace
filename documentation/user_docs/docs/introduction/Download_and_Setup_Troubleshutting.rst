...
If your file layout is different, please run the installation script with the parameters, which specify the location of the Horace and Herbert folders (see ``install_horace`` help on how to do this), and ensure that ***Horace/admin*** and ***Herbert/admin*** folders contains all necessary templates described below.
...
If you do not have write access to the `pathdef.m` file, the script stores the modified copy of the file in the Matlab **$USERPATH** directory. This directory is available and inserted in the beginning Matlab search path modifying Matlab default search path if you start your Matlab session from the link in the OS GUI or from this directory. Some versions of Matlab would not use the same **$USERPATH** directory if you start main Matlab or Matlab parallel workers from other location. This may make ``horace_on`` and ``worker_v2`` scripts not available. Parallel extensions in this case do not report any errors, but would not work. To avoid difficult to identify errors, always modify original `pathdef.m` file, i.e. run Horace installation with administrative access. Otherwise, you may need to always run Matlab from **$USERPATH** directory.
