...
% Read a groups of spectra from the current data source into an array of time-of-flight spectra
%
...
%
%   The arguments is_lo, is_hi, step, can be vectors. The result is equivalent to the concatenation
%   of mspec applied to the arguments element-by-element i.e.
%       >> w = mspec (is_lo, is_hi, step)
%   is equivalent to
%       >> w = [mspec(is_lo(1),is_hi(1),step(1)), mspec(is_lo(2),is_hi(2),step(2)), ...]
%
% Map specification:
% ------------------
...
% -------------------
%   >> w = mspec(...,'mask', mask_array, 'units', unitcode, 'binning', binpar, 'det', det_source, av_mode, 'period', iperiod)
%
...
warr=mspec_core (ns, s, mask, detpar, av_mode, unit, binpar);
