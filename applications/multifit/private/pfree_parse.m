...
%              one per dataset, then pfree_in must be:
%                - An empty argument (which means all parameters are free)
%                - A cell array of vectors, one per function
%                - A single vector (or a cell array with a single vector) which
...
ok=false;
pfree={};
if iscell(pfree_in) && ~isempty(pfree_in)
...
elseif isempty(pfree_in)    % Empty argument; assume all parameters are free
    pfree=cell(size(np));
    for i=1:numel(np)
...
% Input:
% ------
%   pfree_in    Vector describing which parameters are free and which are fixed
%               Can be numeric vector of zeros and ones, or logical vector.
%               If empty, it is assumed that all parameters are free.
...
if (isnumeric(pfree_in)||islogical(pfree_in)) && ~isempty(pfree_in)
