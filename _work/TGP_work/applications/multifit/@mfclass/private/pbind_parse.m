function [ok,mess,ipb,ifunb,ipf,ifunf,R] = pbind_parse(np,nbp,isfore,ifunb_def,bnd)
% Determine if a binding descriptor is valid
%
%   >> [ok,mess,ipf,ifunf,ipb,ifunb,R] = pbind_parse(np,nbp,isfore,ifun_def,bnd)
%
% Input:
% ------
...
elseif ~(numel(ipb)==numel(ipb))
    error('Logic problem. Contact developers.')
end
