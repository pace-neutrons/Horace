function test_tobyfit_rbmnf3 (tf_ver)
% Test Tobyfit versions refining moderator parameter for asingle sqw dataset
%
%   >> test_tobyfit_rbmnf3 (tf_ver)
%
%   tf_ver  If original Tobyfit: =1; if new Tobyfit2: =2
%           If 'setup': edit the sqw file

if isequal(tf_ver,1) || isequal(tf_ver,2)
    setup=false;
elseif isequal(tf_ver,'setup')
    setup=true;
else
...
if tf_ver==1
    ...
    wtmp=tobyfit(w1inc,@van_sqw,[amp,en0,fwhh],[1,1,0],'mc_npoints',mc,'refine_mod',mod_opts,'eval');
    acolor b; dd(w1inc); acolor r; pl(wtmp)
    
    % Good choice of parameters, so start the fit
    [w1fit,pfit,ok,mess,pmodel,ppfit]=tobyfit(w1inc,@van_sqw,[amp,en0,fwhh],[1,1,0],'mc_npoints',mc,'refine_mod',mod_opts,'list',2);
    acolor b; dd(w1inc); acolor r; pl(w1fit)
    
%     % Happy with the fit (ppfit(1)=10.35 +/- 0.28), so redefine the moderator parameters in the main sqw file
%     set_mod_pulse_horace(data_source_crap,pmodel,ppfit);
    ...
    amp=100;  en0=0;   fwhh=0.25;
    kk = kk.set_fun (@van_sqw, [amp,en0,fwhh], [1,1,0]);
