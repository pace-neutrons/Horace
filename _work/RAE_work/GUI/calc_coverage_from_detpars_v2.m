function [xcoords,ycoords,zcoords,pts,ptlabs]=...
    calc_coverage_from_detpars_v2(Ei,hw,omega_min,omega_max,detpar,u,v,alatt,angdeg)
%
...
% 
%This bit then breaks each tube up into 64 sections
%ind=[round(linspace(1,ff(1),8))];
ind=[];
for i=1:2:length(ff)-1
    ind=[ind round(linspace(ff(i),ff(i+1),64))];
end

% ind=ff;

counter=1;
for i=linspace(omega_min,omega_max,50)
