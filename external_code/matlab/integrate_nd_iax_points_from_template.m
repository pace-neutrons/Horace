...
substr_in={'integrate_nd_iax_points_template','iax=1','ndim=2','(ilo,:)','(ihi,:)',...
    '(ml-1,:)','(ml,:)','(mu,:)','(mu+1,:)','(ml:mu-1,:)','(ml+1:mu-1,:)','(ml+1:mu,:)'};

substr_out{1}={'integrate_1d_points',  'iax=1','ndim=1','(ilo)',    '(ihi)',...
    '(ml-1)','(ml)','(mu)','(mu+1)','(ml:mu-1)','(ml+1:mu-1)','(ml+1:mu)'};

substr_out{2}={'integrate_2d_x_points','iax=1','ndim=2','(ilo,:)',  '(ihi,:)',...
    '(ml-1,:)','(ml,:)','(mu,:)','(mu+1,:)','(ml:mu-1,:)','(ml+1:mu-1,:)','(ml+1:mu,:)'};

substr_out{3}={'integrate_2d_y_points','iax=2','ndim=2','(:,ilo)',  '(:,ihi)',...
    '(:,ml-1)','(:,ml)','(:,mu)','(:,mu+1)','(:,ml:mu-1)','(:,ml+1:mu-1)','(:,ml+1:mu)'};

substr_out{4}={'integrate_3d_x_points','iax=1','ndim=3','(ilo,:,:)','(ihi,:,:)',...
    '(ml-1,:,:)','(ml,:,:)','(mu,:,:)','(mu+1,:,:)','(ml:mu-1,:,:)','(ml+1:mu-1,:,:)','(ml+1:mu,:,:)'};

substr_out{5}={'integrate_3d_y_points','iax=2','ndim=3','(:,ilo,:)','(:,ihi,:)',...
    '(:,ml-1,:)','(:,ml,:)','(:,mu,:)','(:,mu+1,:)','(:,ml:mu-1,:)','(:,ml+1:mu-1,:)','(:,ml+1:mu,:)'};

substr_out{6}={'integrate_3d_z_points','iax=3','ndim=3','(:,:,ilo)','(:,:,ihi)',...
    '(:,:,ml-1)','(:,:,ml)','(:,:,mu)','(:,:,mu+1)','(:,:,ml:mu-1)','(:,:,ml+1:mu-1)','(:,:,ml+1:mu)'};
