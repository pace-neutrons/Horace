...
!-----------------------------------------------------------------------------------------------------------------------------------
! Integrate x-axis over a 2D data set with point character along the x-axis between two limits.
...
!
!-----------------------------------------------------------------------------------------------------------------------------------
!	T.G. Perring		2011-07-17		First release. Based very closely single_integrate_1d_points.
!                                       Virtually same as single_integrate_2d_y_points
!
!-----------------------------------------------------------------------------------------------------------------------------------
	implicit none
	...
	integer(i4b) nx, ilo, ihi, i
	real(dp) x1eff, xneff
	real(dp), dimension(size(s,2)) :: s1eff, sneff, e1eff, eneff
...
!	x(ml) to x(mu):
		...
		elseif (mu==ml+1) then	! one complete interval
		    ...
		    val = val + (s(ml+1,:)+s(ml,:))*(x(ml+1)-x(ml))
		    errbar = errbar + (e(ml,:)*(x(ml+1)-x1eff))**2 + (e(mu,:)*(xneff-x(mu-1)))**2
		    do i=ml+1,mu-1
			    val = val + (s(i+1,:)+s(i,:))*(x(i+1)-x(i))
			    errbar = errbar + (e(i,:)*(x(i+1)-x(i-1)))**2
		    end do
		endif
