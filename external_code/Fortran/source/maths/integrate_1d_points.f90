...
!	xout(:)	real		Output intwegration range bin boundaries (run from 1 to NB+1)
!
! OUTPUT:
...
! ---------------------------------------
	...
	nb = size(xout)-1
	if (nb<1) then
		call remark ('Sizes of output integration limits array too small (integrate_1d_points)')
		ierr = ERR
		return
	endif
