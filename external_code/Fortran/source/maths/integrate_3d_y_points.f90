	...
	use maths, only : upper_index, lower_index, single_integrate_3d_y_points
...
!	sout(nx,nb,nz)	real		Output signal
!	eout(nx,nb,nz)	real		Output error bars on signal
!
!-----------------------------------------------------------------------------------------------------------------------------------
...
! ---------------------------------------
	...
	nz = size(s,3)
	if (size(y) /= ny) then
		call remark ('Sizes of y and signal arrays do not correspond (integrate_3d_y_points)')
		ierr = ERR
		return
	endif
	if (size(s) /= size(e)) then
		call remark ('Sizes of signal and error arrays do not correspond (integrate_3d_y_points)')
		ierr = ERR
		return
	endif
	if (ny <= 1) then
		call remark ('Must have at least two data points to perform integration (integrate_3d_y_points)')
		ierr = ERR
		return
	endif
	...
	nb = size(yout)-1
	if (nb<1) then
		call remark ('Size of output integration limits array too small (integrate_3d_y_points)')
		ierr = ERR
		return
	endif
...
! --------------------------
    ...
    do 
        ...
        call single_integrate_3d_y_points(y,s,e,yout(ib),yout(ib+1),ml,mu,sout(:,ib,:),eout(:,ib,:));
