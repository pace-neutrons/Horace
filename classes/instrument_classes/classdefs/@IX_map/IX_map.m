classdef IX_map
    ...
    properties
        ...
        % -----------------------
        ns=0     % Row vector of number of spectra in each workspace. There must be
        % at least one workspace. ns(i)=0 is permitted (it means no spectra in ith workspace)
        s=zeros(1,0);  % Row vector of spectrum indicies in workspaces concatenated together. The
        %          spectrum numbers are sorted into numerically increasing order for
        %          each workspace
        wkno=zeros(1,0); %    Workspace numbers (think of them as the 'names' of the workspaces).
    ...
    methods
        function obj=IX_map(varargin)
            ...
            if nargin==1 && isstruct(varargin{1})
                ...
                wout = varargin{1};
                if optval
                    wout.wkno=wkno_prov;     % only override an existing wkno if one is provided in argument list
                elseif opt
            ...
            elseif narg==1 && isstring(varargin{1})
                ...
                if ~isempty(varargin{1})
                    ...
                    if ~ok, error(mess), end
                    if optval
                        wout.wkno=wkno_prov;     % if ...'wkno') use value read from file, if ...'wkno',val) use provided value
                    elseif ~opt
