...
covar_all = cellfun(@(x)(x.covar(wvec)),dbank_all,'uniformOutput',false);

bob = eff_all; eff_ref = [bob{1}([4,1]), bob{3}(1), bob{4}(3), bob{3}(2), bob{4}(2)];
bob = mean_all; mean_ref = cat(2, bob{1}(:,[4,1]), bob{3}(:,1), bob{4}(:,3), bob{3}(:,2), bob{4}(:,2));
bob = covar_all; covar_ref = cat(3, bob{1}(:,:,[4,1]), bob{3}(:,:,1), bob{4}(:,:,3), bob{3}(:,:,2), bob{4}(:,:,2));


% ----------------------------------------------------------------------------
% Make a detector array
% ----------------------
% Test lots of indexing
...
cov = detarray.covar([4,1,11,16,12,15], wvec);
