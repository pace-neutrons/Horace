function  [this,ok,mess,undef_list] = load_methadata(this,varargin)
...
% Returns modified object and true if all necessary methadata can be loaded
% or false if not. It also returns list of undefined fields
%
%Usage:
%>>[this,ok,mess,undef_list] = load_methadata(this);
%>>[this,ok,mess,undef_list] = load_methadata(this,'-for_powder');
...
if ~ok
    error('RUNDATA:invalid_argument','load_methadata: %s',mess);
end
mess ='';
ok = true;
...
if ~isempty(undef_list)
    ok = false;
    mess = 'some methadata fields can not be defined';
end
