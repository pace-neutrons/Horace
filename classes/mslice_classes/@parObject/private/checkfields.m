...
% Check fields for Horace/Tobyfit parObject objects
%
...
if isequal(fieldnames(wout),fields)
    ...
    if ~size(wout.x2,1)==1, wout.x2=wout.x2(:)'; end
    if ~size(wout.phi,1)==1, wout.phi=wout.phi(:)'; end
    if ~size(wout.azim,1)==1, wout.azim=wout.azim(:)'; end
    if ~size(wout.width,1)==1, wout.width=wout.width(:)'; end
    if ~size(wout.height,1)==1, wout.height=wout.height(:)'; end
    
    % Check numeric, length and any conditions on magnitude
    message_arg_types='Detector group numbers, sample-detector distances, angles and sizes must all be numeric arrays with equal length';
    ...
    [ok,numeric_and_nvals_ok]=check_numeric(wout.x2,ndet,false,true,false);
    if ~ok
        if numeric_and_nvals_ok
            message='Sample-detector distances must all be greater than zero'; return
        else
            message=message_arg_types; return
        end
    end
    ...
    [ok,numeric_and_nvals_ok]=check_numeric(wout.width,ndet,false,true,true);
    if ~ok
        if numeric_and_nvals_ok
    ...
    [ok,numeric_and_nvals_ok]=check_numeric(wout.height,ndet,false,true,true);
    if ~ok
        if numeric_and_nvals_ok
    ...
    message='Fields inconsistent with parObject object';
    return
end
