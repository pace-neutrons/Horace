classdef data_dnd
    % Class defines structure of the data, used by dnd objects
    %
    ...
    properties(Dependent)
        ...
        filepath
        % Title of sqw data structure
        title
        % Lattice parameters for data field (Ang^-1)
        alatt
        % Lattice angles for data field (degrees)
        angdeg
    ...
    properties(Access = private)
        filename_=''   % Name of sqw file that is being read, excluding path
        filepath_=''   % Path to sqw file that is being read, including terminating file separator
        title_   =''   % Title of sqw data structure
        alatt_   =[1,1,1] % Lattice parameters for data field (Ang^-1)
        angdeg_  =[90,90,90]% Lattice angles for data field (degrees)
        
        % A projection, which defines transformation between pixels and sqw
        % objects system of coordinates
        proj_ = projection();
        
        s_=[]          %Cumulative signal.  [size(data.s)=(length(data.p1)-1, length(data.p2)-1, ...)]
        e_=[]          %Cumulative variance [size(data.e)=(length(data.p1)-1, length(data.p2)-1, ...)]
        npix_=[]       %No. contributing pixels to each bin of the plot axes.
        %             [size(data.pix)=(length(data.p1)-1, length(data.p2)-1, ...)]
    ...
    methods
        ...
        %------------------------------------------------------------------
        function obj = data_dnd(varargin)
            if nargin>0 && isa(varargin{1},'data_dnd') % handle shallow copy constructor
        ...
        function fpath = get.filepath(obj)
            fpath = obj.filepath_;
        end
        ...
        function obj = set.filename(obj,val)
            check_string_or_empty_string_(val,'filename');
            obj.filename_ = val;
        end
        function obj = set.filepath(obj,val)
            check_string_or_empty_string_(val,'filepath');
            obj.filepath_ = val;
        end
        function obj = set.title(obj,val)
            check_string_or_empty_string_(val,'title');
            obj.title_ =val;
        end
