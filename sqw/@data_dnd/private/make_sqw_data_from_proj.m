...
%
% $Revision: 1460 $ ($Date: 2017-04-03 22:35:33 +0100 (Mon, 03 Apr 2017) $)



% Check projection
if isstruct(proj_in)
    proj = projection(proj_in);
elseif isa(proj_in,'aProjection')
    proj = proj_in;
else
...
data.filepath = '';
data.title = '';
data.alatt=lattice.alatt; %(1:3);
data.angdeg=lattice.angdeg; %lattice(4:6);
data.proj = proj;
...
% data.p=p;
% data.dax=1:ndim;
data.s=zeros(proj.grid_size);
data.e=zeros(proj.grid_size);
data.npix=ones(proj.grid_size);
