function [mess,position,npixtot,type] = write_sqw_data (fid, data, opt, infiles, npixstart, pixstart, run_label)
...
% contributing sqw files, as is recorded in the corresponding header block (see write_sqw_header).
...
type_in = sqw_type(data);
type = type_in;
if strcmpi(type_in,'b')||strcmpi(type_in,'b+')
    if exist('opt','var')
        disp('WARNING: options specified in write_sqw_data ignored for this sqw type')
    end
...
elseif strcmpi(type_in,'a-')
    ...
    error('logic error in write_sqw_data')
end
...
if strcmpi(type_in,'a')||strcmpi(type_in,'a-')
    ...
    elseif pix_from_file
        ...
        position.pix=ftell(fid);
        type='a';
        mess = write_sqw_data_pix_from_file (fid, infiles, npixstart, pixstart, npix_cumsum, run_label);
    else
        type='a-';
    end
end
