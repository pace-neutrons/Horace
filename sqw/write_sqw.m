function [mess,position,npixtot,type] = write_sqw (outfile,main_header,header,detpar,data,varargin)
% Write an sqw data structure to file
%
%   >> [mess, position, npixtot, type] = write_sqw (outfile, main_header, header, detpar, data)
%   >> [mess, position, npixtot, type] = write_sqw (outfile, main_header, header, detpar, data, '-nopix')
%   >> [mess, position, npixtot, type] = write_sqw (outfile, main_header, header, detpar, data, '-pix')
%   >> [mess, position, npixtot, type] = write_sqw (outfile, main_header, header, detpar, data, '-pix',...
...
% Initialise output
position = struct('main_header',0,'header',zeros(nfiles,1),'detpar',0,'data',0,'s',0,'e',0,'npix',0,'pix',0);
npixtot = [];
type='';
...
mess = write_sqw_main_header (fout, main_header);
...
if isstruct(header) % should be a single header, as a data structure
    ...
    mess = write_sqw_header (fout, header);
    if ~isempty(mess); if close_file; fclose(fout); end; return; end
    ...
    for i=1:nfiles
        ...
        mess = write_sqw_header (fout, header{i});
        if ~isempty(mess); if close_file; fclose(fout); end; return; end
    end
end
...
mess = write_sqw_detpar (fout, detpar);
if ~isempty(mess); if close_file; fclose(fout); end; return; end

% Write data
position.data=ftell(fout);
[mess,position_data,npixtot,type] = write_sqw_data (fout, data, varargin{:});
if ~isempty(mess); if close_file; fclose(fout); end; return; end
