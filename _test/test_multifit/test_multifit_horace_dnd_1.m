classdef test_multifit_horace_dnd_1 < TestCaseWithSave
    % Performs some tests of fitting to Horace dnd objects using multifit_sqw and other functions.
    % Optionally writes results to output file
    %
    %   >> runtests test_multifit_horace_dnd_1  % Compares with previously saved results in
    %                                       % test_multifit_horace_1_output.mat
    %                                       % in the same folder as this function
    %
    %   >> test_multifit_horace_dnd_1('-save')  % Save to test_multifit_horace_1_output.mat
    %                                       % in the Matlab temporary folder (copy to
    %                                       % the same folder as this function after)
    %
    %   >> runtests test_multifit_horace_dnd1:<test_func> % run a particular test from this
    %                                       % this test suite
    %
    ...
    methods
        function this = test_multifit_horace_dnd_1(name)
            % Construct object
            output_file = 'test_multifit_horace_1_dnd_output.mat';
            this = this@TestCaseWithSave(name, output_file);
            
            % Read in data
            data_dir = fileparts(mfilename('fullpath'));
            
            %CHANGED to make new sqw class work with this
            % w1data and w2data are sqw objects (actually sqw_old objects
            % but stored with the sqw class name.) To use them as dnd
            % objects for this test, now cast them to d1d (as they are
            % 1-dimensional):
            sqw_w1data = sqw(fullfile(data_dir,'w1data.sqw')); %was read_sqw, gave sqw_old
            sqw_w2data = sqw(fullfile(data_dir,'w2data.sqw')); %was read_sqw, gave sqw_old
            this.w1data = d1d(sqw_w1data);
            this.w2data = d1d(sqw_w2data);
            this.win=[this.w1data,this.w2data];     % combine the two cuts into an array of d1d objects and fit
        ...
        function this = test_fit_two_datasets(this)
            ...
            %multifit does not seem to distinguish between sqw and dnd in
            %this call
        ...
        % ------------------------------------------------------------------------------------------------
        function this=DISABLED_test_fit_array_of_datasets(this)
            ...
            % *** NEEDS FIT_SQW TO BE IMPLEMENTED
...
%             this=save_or_test_variables(this,wfit_single1,wfit_single2,wfit_single12);
        ...
        % ------------------------------------------------------------------------------------------------
        function this=DISABLED_test_fit_test_fit_array_of_datasets_2(this)
            % Example of fitting single datasets, and independently fitting an array of datasets
            % This exercises multifit_sqw for single datasets and compares with fit_sqw for an array of datasets
            
            % *** CODE NEEDS TO BE EDITED ONCE FIT_SQW CREATED
...
%             this.tol=tol;
        ...
        % ------------------------------------------------------------------------------------------------
        function this=DISABLED_test_fit_array_of_datasets_3(this)
            % Example of fitting single datasets, and independently fitting an array of datasets
            % This exercises multifit_sqw_sqw for single datasets and fit_sqw_sqw for an array of datasets
            
            % *** CODE NEEDS TO BE EDITED ONCE FIT_SQW_SQW CREATED
