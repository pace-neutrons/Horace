...
%
% The data is created in the folder returned by the function tmp_dir.
%
% Author: T.G.Perring
...
sqw_file=fullfile(tmp_dir,'test_mslice_horace.sqw');
efix=200;
emode=1;
alatt=[5,5,4.58];
angdeg=[90,90,90];
...
save(s1q,fullfile(tmp_dir,'s1q.cut'));
save(s1e,fullfile(tmp_dir,'s1e.cut'));
save(s2qq,fullfile(tmp_dir,'s2qq.slc'));
save(s2qe,fullfile(tmp_dir,'s2qe.slc'));
...
% -----------------------------------------------------------------------------
files={fullfile(tmp_dir,'w1q.sqw'),fullfile(tmp_dir,'w1e.sqw'),...
       fullfile(tmp_dir,'w2qq.sqw'),fullfile(tmp_dir,'w2qe.sqw'),...
       fullfile(tmp_dir,'s1q.cut'),fullfile(tmp_dir,'s1e.cut'),...
       fullfile(tmp_dir,'s2qq.slc'),fullfile(tmp_dir,'s2qe.slc')};
...
% ---------------------------------------------
scalc=spe(wcalc);
spe_file=fullfile(tmp_dir,'test_mslice_horace.spe');
save(scalc,spe_file)
...
mslice_2d([-0.82,0.04,1.26],[-0.9,0.04,1.3],[30,35],'range',[0,2],'plot',0,'file',fullfile(tmp_dir,'ms_s2qq.slc'))
mslice_2d([0.3,0.04,0.7],[0.2,0.4],[15,0,60],'range',[0,2],'plot',0,'file',fullfile(tmp_dir,'ms_s2qe.slc'))

mslice_1d([0.3,0.04,0.7],[0.2,0.4],[30,35],'range',[0,3],'plot','file',fullfile(tmp_dir,'ms_s1q.cut'));
mslice_1d([0.4,0.5],[0.2,0.4],[15,0,60],'range',[0,3],'plot','file',fullfile(tmp_dir,'ms_s1e.cut'));
