classdef test_gen_sqw_accumulate_sqw_mex < TestCaseWithSave
    ...
    methods
        ...
        %
        function [skip,cur_mex,pref]=setup_mex_mode(this,varargin)
            hc = hor_config;
            pref = this.prefix;
            skip = false;
            if this.skip_tests
        ...
        function this=build_test_files(this,varargin)
            %-------------------------------------------------------------
            [skip,cur_mex,pref]=this.setup_mex_mode(varargin{:});
            if skip
                return
            end
            ...
            % various amount of noise to various detectors according to the
            % ordering
            hc = hor_config();
            um = hc.use_mex;
            hc.use_mex = false;
        ...
        function this=test_gen_sqw(this)
            ...
            set(hpc_config,'accum_in_separate_process',false,'use_mex_for_combine',false);
            %-------------------------------------------------------------
