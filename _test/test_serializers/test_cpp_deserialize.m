classdef test_cpp_deserialize < TestCase
    ...
    methods
        function this=test_cpp_deserialize(varargin)
            if nargin>0
                name = varargin{1};
            else
        ...
        function test_ser_sample(this)
            ...
            bytes = c_serialize(sam1);
            sam1rec = c_deserialize(bytes);
            assertEqual(sam1,sam1rec);

            sam2=IX_sample(true,[1,1,0],[0,0,1],'cuboid',[0.04,0.03,0.02]);

            bytes = c_serialize(sam2);
            sam2rec = c_deserialize(bytes);
        ...
        function test_ser_instrument(this)
            ...
            bytes = c_serialize(inst1);
            inst1rec = c_deserialize(bytes);
            assertEqual(inst1,inst1rec);


            inst2=create_test_instrument(56,300,'s');
            inst2.flipper=true;
            bytes = c_serialize(inst2);
            inst2rec = c_deserialize(bytes);
            assertEqual(inst2,inst2rec );

            inst3=create_test_instrument(195,600,'a');
            inst3.filter=[3,4,5];
            bytes = c_serialize(inst3);
            inst3rec = c_deserialize(bytes);
        ...
        function test_ser_datamessage(this)
            ...
            ser = c_serialize(test_obj);
            test_obj_rec = c_deserialize(ser);
        ...
        function test_ser_struct_null(this)
            ...
            test_struct = struct([]);
            ser =  c_serialize(test_struct);
            test_struct_rec = c_deserialize(ser);
        ...
        function test_ser_real_sparse_null(this)
            ...
            test_sparse = sparse([],[],[]);
            ser =  c_serialize(test_sparse);
            test_sparse_rec = c_deserialize(ser);
        ...
        function test_ser_function_handle(this)
            ...
            ser = c_serialize(test_func);
            test_func_rec = c_deserialize(ser);
        ...
        function test_ser_cell_null(this)
            ...
            test_cell = {};
            ser =  c_serialize(test_cell);
            test_cell_rec = c_deserialize(ser);
