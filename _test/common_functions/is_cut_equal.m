...
function [f1, f2, proj, pN, cutArgs, tol] = parse_args(varargin)
    p = inputParser();
    addRequired(p, 'f1', @validate_sqw_or_path);
    addRequired(p, 'f2', @validate_sqw_or_path);
    addOptional(p, 'proj', projection(), @validate_proj);
    addOptional(p, 'p1', [], @(x)(validateattributes(x, {'numeric'}, {'nonempty', 'vector'})));
    addOptional(p, 'p2', [], @(x)(validateattributes(x, {'numeric'}, {'nonempty', 'vector'})));
    ...
    addParameter(p, 'tol', [0, 0], @(x)(validateattributes(x, {'numeric'}, {'vector'})))
    parse(p, varargin{:});
