classdef boxClass
    ...
    methods
        function obj = boxClass (position, sides)
            % Construct an instance of boxClass
            %
        ...
        %------------------------------------------------------------------
        function [ok, mess] = validate_points_in_box (obj, x1col, x1row, x12, ...
                shift1col, shift1row, shift12)
            % Test points as could be generated by the method rand_position
            % lie inside the box, with optional shifts.
            %
            %   >> [ok, mess] = validate_points_in_box (obj, x1col, x1row, x12)
            %
            %   >> [ok, mess] = validate_points_in_box (obj, x1col, x1row, x12, ...
            %                               shift1col, shift1row, shift12)
            %
            ...
            % Determine size of stack. Assume input is valid output from rand_position
            sz = size(x1col);
            sz = sz(2:end);
            if numel(sz)==1
                sz = [1,sz];    % account for 1D stack
            end
            
            % Confirm all data has correct size
            if ~isequal(size(x1col), size_array_stack ([3,1], sz))
                error('HERBERT:boxClass:invalid_argument', ...
                    'Size of x1col is inconsistent with other point data')
            end
            if ~isequal(size(x1row), size_array_stack ([1,3], sz))
                error('HERBERT:boxClass:invalid_argument', ...
                    'Size of x1row is inconsistent with other point data')
            end
            if ~isequal(size(x12), size_array_stack ([3,2], sz))
                error('HERBERT:boxClass:invalid_argument', ...
                    'Size of x12 is inconsistent with other point data')
            end
