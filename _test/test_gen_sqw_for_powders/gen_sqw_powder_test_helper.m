...
for i=1:nfiles
    ...
    data=w.data;
    data.pix.coordinates(1:3,:)=[sqrt(sum(data.pix.coordinates(1:3,:).^2,1));zeros(2,data.pix.num_pixels)];
    data.urange(:,1:3)=[min(data.pix.coordinates(1:3,:),[],2)';max(data.pix.coordinates(1:3,:),[],2)'];
