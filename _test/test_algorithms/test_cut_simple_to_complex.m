classdef test_cut_simple_to_complex < TestCaseWithSave
    ...
    methods
        ...
        function test_simple1Dcut_from_cut_tricl(obj)
            sqw_obj = obj.sqw_4D_tricl;
            ...
            assertEqualToTolWithSave(obj,cut1_fc,'-ignore_date', ...
                'ignore_str',1,'tol',1.e-9);
        end
        
        function test_simple1Dcut_from_newly_generated_sqw_object_tricl(obj)
            sqw_obj = obj.sqw_4D_tricl;
            pr = struct('u',[1,0,0],'v',[0,1,0]);
            cut1D_ng = cut(sqw_obj,pr,[-1.5,0.02,1.5],[-0.1,0.1],[-0.1,0.1],[-5,5]);
            
            assertEqualToTolWithSave(obj,cut1D_ng,'-ignore_date', ...
                'ignore_str',1,'tol',1.e-9);
        end
        
        function test_simple1Dcut_from_cut_ortho(obj)
            sqw_obj = obj.sqw_4D_ortho;
            pr = struct('u',[1,0,0],'v',[0,1,0]);
            ref_file = fullfile(obj.this_dir,'test_cut_simple_to_complex_output.mat');
            if is_file(ref_file)
                ld = load(ref_file);
        ...
        function test_simple1Dcut_from_newly_generated_sqw_object_ortho(obj)
            sqw_obj = obj.sqw_4D_ortho;
            pr = struct('u',[1,0,0],'v',[0,1,0]);
            cut1D_ng = cut(sqw_obj,pr,[-1.5,0.02,1.5],[-0.1,0.1],[-0.1,0.1],[-5,5]);
        ...
        function test_nranges_tricl(obj)
        ...
        function test_nranges_ortho(obj)
