...
if ~isequal(w,wtmp), error('save followed by read not an identity'), end
...
if ~ok, error('Map constructor should have failed but did not'), end
...
if ~isequal(wtmp,w2), error('wkno not correctly assigned'), end

w3=IX_map('map_15_1st_empty.map','wk',11:25);
wtmp=w1; wtmp.wkno=11:25; 
if ~isequal(wtmp,w3), error('wkno not correctly assigned'), end
...
wcomb=combine(w1);
if ~isequal(w1,wcomb), error('Error combining two maps'), end

% COmbine two maps
wcomb=combine(w1,w2);
if ~isequal(wref,wcomb), error('Error combining two maps'), end
...
if ~ok, error('Should have failed because sahred spectra'), end

% Combine workspaces with names
wcomb=combine(w1nam,w2nam,'wkno');
if ~isequal(wrefnam,wcomb), error('Error combining two maps'), end
...
if ~isequal(mtotnam,wcomb), error('Error combining three maps'), end

wcomb=combine(m1,m2,m3);
if ~isequal(mtot,wcomb), error('Error combining three maps'), end
...
if ~isequal(wmsk,wmskref), error('Error masking map object'), end
