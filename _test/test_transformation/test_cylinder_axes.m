classdef test_cylinder_axes < TestCase
    % Tests for main operations of the cylinder_axes
    ...
    methods
        function obj=test_cylinder_axes(varargin)
            if nargin<1
                name = 'test_cylinder_axes';
            else
                name = varargin{1};
            end
        ...
        function test_bin_volume_array(~)
            dbr =[...
                0.5, 45,  0, 0;...
                1.5, 46, 90, 10];
            ab = cylinder_axes('img_range',dbr,'nbins_all_dims',[10,10,10,10]);
        ...
        function test_bin_volume_single(~)
            dbr = [...
                1  , 45,  0, 0;...
                1.1, 46, 90, 10];
            ab = cylinder_axes('img_range',dbr,'nbins_all_dims',[1,1,10,10]);

            vol = ab.get_bin_volume();
            assertEqual(numel(vol),100);
            % single bin volume Int(r*dR*dPhi*dz)
            ref_vol = 0.5*0.1*(1+1.1)*1*9*pi/180*1;
            assertEqualToTol(vol(1),ref_vol,1.e-8);
        end
        function test_bin_volume_single_large(~)
            dbr = ...
                [0, -1,-180, 0;...
                1 ,  1, 180, 10]; % half a sphere
            ab = cylinder_axes('img_range',dbr,'nbins_all_dims',[1,1,1,10]);

            vol = ab.get_bin_volume();
            assertEqual(numel(vol),1);
            ref_vol = 2*pi;  % half a sphere volume
        ...
        function test_angular_is_rad_in_constructor(~)
            ...
            bin0 = {[dbr(1,1),0.05,dbr(2,1)];[dbr(1,2),0.1,dbr(2,2)];...
                [dbr(1,3),dbr(2,3)];[dbr(1,4),1,dbr(2,4)]};
            ab = cylinder_axes(bin0{:},'angular_unit_is_rad','r');
            assertEqual(ab.img_range,[ ...
                0,    -1.05,-pi,-10.5; ...
                10.05, 2.05, pi, 50.5])
            assertTrue(ab.angular_unit_is_rad);
            ab.angular_unit_is_rad = false;
            assertEqual(ab.img_range,[ ...
                0,    -1.05,-180,-10.5; ...
                10.05, 2.05, 180, 50.5])
        ...
        function test_bin_edges_provided_4D(~)
            ...
            bin0 = {[dbr(1,1),0.5,dbr(2,1)];[dbr(1,2),1,dbr(2,2)];...
                [dbr(1,3),dbr(2,3)];[dbr(1,4),1,dbr(2,4)]};
            ab = cylinder_axes(bin0{:},'single_bin_defines_iax',[true,false,false,true]);
        ...
        function test_cylinder_axes_change_angular_range_in_parts_logical(~)
            ab = cylinder_axes(4);
            assertEqual(ab.dimensions,4);
            assertEqual(ab.angular_unit_is_rad,false);
            assertEqual(ab.img_range,[0,-1,-180,-1;1,1,180,1])
            ab.angular_unit_is_rad = true;
            assertEqual(ab.angular_unit_is_rad,true);
            assertEqual(ab.img_range,[0,-1,-pi,-1;1,1,pi,1])
            ab.angular_unit_is_rad = false;
            assertEqual(ab.angular_unit_is_rad,false);
            assertEqual(ab.img_range,[0,-1,-180,-1;1,1,180,1])
        end

        function test_cylinder_axes_change_angular_range(~)
            ab = cylinder_axes(3);
            assertEqual(ab.dimensions,3);
            assertEqual(ab.angular_unit_is_rad,false);
            assertEqual(ab.img_range,[0,-1,-180,0;1,1,180,0])
            ab.angular_unit_is_rad = 'd';
            assertEqual(ab.angular_unit_is_rad,false);
            assertEqual(ab.img_range,[0,-1,-180,0;1,1,180,0])
            ab.angular_unit_is_rad = 'r';
            assertEqual(ab.angular_unit_is_rad,true);
            assertEqual(ab.img_range,[0,-1,-pi,0;1,1,pi,0])
        end
        %
        function test_cylinder_axes_0D_explicit(~)
            ab = cylinder_axes(0);
        ...
        %
        function test_cylinder_axes_1D_explicit(~)
            ab = cylinder_axes(1);
        ...
        %
        function test_cylinder_axes_2D_explicit(~)
            ab = cylinder_axes(2);
        ...
        %
        function test_cylinder_axes_4D_explicit(~)
            ab = cylinder_axes(4);
            assertEqual(ab.dimensions,4);
            assertEqual(ab.nbins_all_dims,ones(1,4))
