classdef test_projection_class<TestCase
    ...
    methods
        ...
        function test_constructor(this)
            ...
            assertEqual(proj.type,'ppr') %TODO: Should it be 'aaa'?
            assertTrue(isempty(proj.w))
            
            f = @()projection([0,1,0]);
            assertExceptionThrown(f,'APROJECTION:invalid_argument');
            
            proj = projection(10,[1,1,1,1;2,2,2,10],[0,1,0],[1,0,0]);
            assertElementsAlmostEqual(proj.u,[0,1,0])
            assertElementsAlmostEqual(proj.v,[1,0,0])
            assertTrue(isempty(proj.w))
            
            prja = projaxes([1,0,0],[0,1,1],[0,-1,1]);
            proj = projection(10,[1,1,1,1;2,2,2,10],prja);
