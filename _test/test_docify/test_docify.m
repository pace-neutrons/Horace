...
mkgpath('meta_docs',fullfile(rootpath,'docify_files'));   % define a global path

source_undocified = fullfile(rootpath,'source_undocified');
source_docified = fullfile(rootpath,'source_docified');

% Copy test files to temporary folder
tmpfolder = fullfile(tempdir,'test_docify');
try rmdir(tmpfolder,'s'), catch, end   % delete, if present and if can
